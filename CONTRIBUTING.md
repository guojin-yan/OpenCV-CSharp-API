...
- Keep real runtime pack inputs honest: `pack.yml` does not build real runtime inputs; when `validate_synthetic_runtime=false`, real input paths must already exist on the selected runner or come from `real_runtime_artifact_run_id` downloading a neutral `runtime-input-<rid>-<profile>` artifact, and real publishable runtime packages require `SyntheticRuntimeInputs=false` provenance plus release preflight.
...
- Keep DocFX configuration and generated API documentation surfaces on `JYPPX.OpenCvSharp.*`, `src/OpenCvSharp/OpenCvSharp.csproj`, `docs/api`, and `docs/_site`.
...
- Keep Actions usage local-first and manually dispatched: run invariant, build, pack, and DocFX checks locally before using hosted runners; dispatch ordinary managed/native/docs verification only when needed on `grape-yan/OpenCV-CSharp-API`; reserve `guojin-yan/OpenCV-CSharp-API` Actions for authoritative packaging, publication, public-feed verification, Release creation, and explicitly approved Pages deployment.
- Keep `build-managed`, `build-native`, and `docs` free of automatic `push` and `pull_request` triggers so authoritative pushes do not create skipped runs or consume avoidable Actions quota.
- Keep every managed and runtime NuGet package branded with root `logo.jpg` from `nuget/logo.jpg` and root `README.md` from the repository English README; nuspec `icon` and `readme` metadata must point to those exact package-root files.
...
- Do not copy source code from JYPPX.OpenCvSharp, Emgu CV, or other projects.
