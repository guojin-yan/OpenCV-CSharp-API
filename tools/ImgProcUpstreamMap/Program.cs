...
internal static class Program
{
    ...
    private sealed class ClassificationDocument
    {
        ...
        public string ClaimedSlice { get; set; } = "opencv2/imgproc.hpp declarations emitted by OpenCV hdr_parser.py";
    ...
    private sealed class MappingSummary
    {
        ...
        public string Generator { get; init; } = "tools/ImgProcUpstreamMap";
        public string UpstreamOpenCvVersion { get; init; } = "5.0.0";
        public string ClaimedSlice { get; init; } = "opencv2/imgproc.hpp declarations emitted by OpenCV hdr_parser.py";
    ...
    private sealed class FamilyInventory
    {
        ...
        public int ManagedPublicMemberAdditionCount { get; init; } = 174;
    ...
    private sealed class FamilyOperation
    {
        ...
        public string FocusedTest { get; init; } = "tests/OpenCvSharp.Tests/ImgProc/ImgProcUpstreamParityTests.cs";
        ...
        public string Guide { get; init; } = "docs/articles/imgproc-upstream-parity-guide.md";
    }
    ...
    private static int Main(string[] args)
    {
        try
        {
            ...
            Console.WriteLine("IMGPROC_UPSTREAM_MAP_BASE_VALIDATION_OK");
            ...
            Console.WriteLine(
                "IMGPROC_UPSTREAM_MAP_OK declarations={0} callable={1} implemented={2} missing={3} omitted={4} fixtures=10 sha256={5} mode={6}",
        ...
        IReadOnlyList<ManagedMember> managedMembers)
    {
        ...
        foreach (RawDeclaration declaration in raw.Declarations)
        {
            ...
            row.NativeEntrypoints = FindNativeEvidence(owner, symbol, nativeEntrypoints).ToList();
        ...
        IReadOnlyList<ManagedMember> members)
    {
        ...
        if (owner.Length > 0)
        {
            string typeName = "JYPPX.OpenCvSharp.ImgProc." + owner;
    ...
    {
        string symbolToken = SnakeCase(symbol);
    ...
    {
        Require(raw.SchemaVersion == 1 && raw.Generator == "tools/ImgProcUpstreamMap/extract_imgproc.py" && raw.UpstreamOpenCvVersion == "5.0.0", "Raw extraction identity drifted.");
        Require(raw.DeclarationCount == 203 && raw.Declarations.Count == 203, "Raw extraction must contain exactly 203 declarations.");
        Require(raw.Declarations.Count(value => value.Kind == "enum") == 29 && raw.Declarations.Count(value => value.Kind == "class") == 7 && raw.Declarations.Count(value => value.Kind == "callable") == 167, "Raw declaration kind counts drifted.");
    ...
    {
        var builder = new StringBuilder();
        builder.AppendLine("# OpenCV 5.0.0 ImgProc upstream-to-native-to-managed map");
        builder.AppendLine("schema-version=1");
        builder.AppendLine("claimed-slice=opencv2/imgproc.hpp declarations emitted by OpenCV hdr_parser.py");
    ...
    private static FamilyInventory BuildFamilyInventory(RawDocument raw, ClassificationDocument classifications)
    {
        var definitions = new[]
        {
            new
            {
                Id = "generalized-hough-object-model",
                Rationale = "Complete the Ballard and Guil generalized Hough lifecycle, template, detection, and property hierarchy.",
                FocusedTest = "tests/OpenCvSharp.Tests/ImgProc/ImgProcUpstreamParityTests.cs",
                Match = (Func<RawDeclaration, bool>)(value => value.Name.Contains("GeneralizedHough", StringComparison.Ordinal))
            },
            new
            {
                Id = "color-conversion-and-visualization",
                Rationale = "Connect two-plane YUV conversion, Bayer demosaicing, weighted blending, and built-in or user color maps.",
                FocusedTest = "tests/OpenCvSharp.Tests/ImgProc/ImgProcUpstreamParityTests.cs",
                Match = (Func<RawDeclaration, bool>)(value => value.Name is "cv.cvtColorTwoPlane" or "cv.demosaicing" or "cv.blendLinear" or "cv.applyColorMap" or "cv.ColormapTypes")
            },
            new
            {
                Id = "filter-gradient-and-masked-threshold",
                Rationale = "Complete an image-preparation workflow with stack blur, spatial derivatives, and masked thresholding.",
                FocusedTest = "tests/OpenCvSharp.Tests/ImgProc/ImgProcUpstreamParityTests.cs",
                Match = (Func<RawDeclaration, bool>)(value => value.Name is "cv.stackBlur" or "cv.spatialGradient" or "cv.thresholdWithMask")
            },
            new
            {
                Id = "drawing-markers-polygons-and-text-scale",
                Rationale = "Complete common overlay primitives with marker shapes, convex filling, and pixel-height font scaling.",
                FocusedTest = "tests/OpenCvSharp.Tests/ImgProc/ImgProcUpstreamParityTests.cs",
                Match = (Func<RawDeclaration, bool>)(value => value.Name is "cv.MarkerTypes" or "cv.drawMarker" or "cv.fillConvexPoly" or "cv.getFontScaleFromHeight")
            },
            new
            {
                Id = "calibration-sampling-and-coordinate-workflows",
                Rationale = "Connect image undistortion, inverse rectification, sub-pixel sampling, polar remapping, and camera-axis overlays.",
                FocusedTest = "tests/OpenCvSharp.Tests/ImgProc/ImgProcRemainingParityTests.cs",
                Match = (Func<RawDeclaration, bool>)(value => value.Name is "cv.undistort" or "cv.initInverseRectificationMap" or "cv.fisheye.undistortImage" or "cv.getRectSubPix" or "cv.warpPolar" or "cv.drawFrameAxes")
            },
            new
            {
                Id = "accumulation-registration-and-matching-workflows",
                Rationale = "Connect temporal accumulators, phase correlation, Hanning windows, earth mover distance, and template matching.",
                FocusedTest = "tests/OpenCvSharp.Tests/ImgProc/ImgProcRemainingParityTests.cs",
                Match = (Func<RawDeclaration, bool>)(value => value.Name is "cv.accumulate" or "cv.accumulateSquare" or "cv.accumulateProduct" or "cv.accumulateWeighted" or "cv.phaseCorrelate" or "cv.phaseCorrelateIterative" or "cv.createHanningWindow" or "cv.wrapperEMD" or "cv.matchTemplate")
            },
            new
            {
                Id = "segmentation-and-link-runs-workflows",
                Rationale = "Connect watershed, pyramid mean shift, GrabCut, and both link-runs contour overloads with owned managed outputs.",
                FocusedTest = "tests/OpenCvSharp.Tests/ImgProc/ImgProcRemainingParityTests.cs",
                Match = (Func<RawDeclaration, bool>)(value => value.Name is "cv.watershed" or "cv.pyrMeanShiftFiltering" or "cv.grabCut" or "cv.findContoursLinkRuns")
            },
            new
            {
                Id = "font-face-object-and-rendering",
                Rationale = "Complete the OpenCV 5 FontFace lifecycle, variable-font instance transfer, and custom-font text measurement and rendering.",
                FocusedTest = "tests/OpenCvSharp.Tests/ImgProc/ImgProcRemainingParityTests.cs",
                Match = (Func<RawDeclaration, bool>)(value => value.Name == "cv.PutTextFlags" || value.Name.Contains("cv.FontFace", StringComparison.Ordinal) || value.Identity.Contains("FontFace fface", StringComparison.Ordinal))
            }
        };
        ...
        foreach (var definition in definitions)
        {
            ...
            Require(family.Declarations.Count > 0, "Selected ImgProc family has no upstream declarations: " + family.Id);
        ...
        IReadOnlyList<ManagedMember> managedMembers)
    {
        ...
        void Reject(string name, string expected, Action action)
        {
            ...
            {
                action();
                throw new InvalidDataException("Negative ImgProc map fixture was accepted: " + name);
            }
        ...
        Reject("undocumented omission", "documented reason", () => { var value = CopyClassifications(); ClassificationRow row = value.Declarations.First(item => item.Classification == "intentionally-omitted"); row.Reason = string.Empty; Validate(raw, value, options, nativeEntrypoints, managedMembers, false); });
        Reject("false implementation", "requires callable native and managed evidence", () => { var value = CopyClassifications(); ClassificationRow row = value.Declarations.First(item => item.Classification == "intentionally-omitted"); row.Classification = "implemented"; row.NativeEntrypoints.Clear(); row.ManagedMembers.Clear(); Validate(raw, value, options, nativeEntrypoints, managedMembers, false); });
        ...
        Require(rejected == 10, "ImgProc map negative fixture count drifted.");
    }
