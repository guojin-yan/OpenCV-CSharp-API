...
internal static class Program
{
        ...
        IReadOnlyList<ManagedMember> members)
    {
        ...
        {
            candidates = candidates.Where(value => value.TypeName == "JYPPX.OpenCvSharp.VideoIO.VideoStreamReader");
        }
        else if (owner == "VideoCapture" && managedName == "Open")
        {
            candidates = candidates.Where(value => value.TypeName is "JYPPX.OpenCvSharp.VideoIO.VideoCapture" or "JYPPX.OpenCvSharp.VideoIO.VideoCaptureExtensions");
        }
        else if (owner.Length > 0)
        {
            string typeName = "JYPPX.OpenCvSharp.VideoIO." + owner;
        ...
        if ((owner is "VideoCapture" or "VideoWriter") && (managedName is ".ctor" or "Open"))
        {
            ...
            candidates = candidates.Where(value =>
            {
                ...
                if (owner == "VideoWriter")
                {
                    ...
                    bool evidenceHasApiPreference = value.Evidence.Contains("JYPPX.OpenCvSharp.VideoIO.VideoCaptureAPIs apiPreference", StringComparison.Ordinal);
                ...
                if (hasStreamReader)
                {
                    return value.Evidence.Contains("JYPPX.OpenCvSharp.VideoIO.VideoStreamReader reader", StringComparison.Ordinal);
                }
                return value.Evidence.EndsWith(".ctor()", StringComparison.Ordinal);
            });
        }
        ...
        {
            candidates = candidates.Where(value => value.Evidence.Contains("System.Boolean " + managedName + "(JYPPX.OpenCvSharp.Core.Mat", StringComparison.Ordinal));
        }
