...
$manifestPath = Join-Path $nativeRoot "generated/native_abi_manifest.txt"
$miniManifestPath = Join-Path $nativeRoot "generated/native_abi_mini_manifest.txt"
$miniHeaderPaths = @(
    "core/decomp.h"
    "core/mat.h"
    "core/operations.h"
...
$declarationMarkerPattern = "OPENCV_CSHARP_EXTERN_C\s+OPENCV_CSHARP_API\s+"
...
$manifestLines = @(
    "# Generated by scripts/Generate-NativeAbiManifest.ps1. Do not edit."
    ...
    $sortedDeclarations |
        ForEach-Object {
            "$($_.Name)|$($_.ReturnType)|$($_.ParameterCount)|$($_.Header)"
        }
)
...
Write-GeneratedText -Path $manifestPath -Content ($manifestLines -join "`n")
Write-GeneratedText -Path $miniManifestPath -Content ($miniManifestLines -join "`n")

$mode = if ($Check) { "Verified" } else { "Generated" }
Write-Host "$mode $manifestPath"
Write-Host "$mode $miniManifestPath"
