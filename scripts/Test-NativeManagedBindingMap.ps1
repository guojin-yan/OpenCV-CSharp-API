...
function Test-BindingMapDocument {
    ...
    Assert-True -List $List -Condition ($bindingLines.Count -eq 2452 -and $actualEntrypoints.Count -eq 2452) -Path $Path -Issue "Binding-map native entry count drifted" -Text "actual=$($actualEntrypoints.Count) expected=2452"
    ...
    Assert-True -List $List -Condition ([int]$Summary.nativeFunctionCount -eq 2452 -and [int]$Summary.managedEntryPointCount -eq 2452 -and [int]$Summary.managedBoundCount -eq 2452) -Path $Path -Issue "Binding-map summary parity counts drifted"
...
Write-Host "NATIVE_MANAGED_BINDING_MAP_CONTRACT_OK native=2452 bound=2452 unbound=0 managed_only=0 imports=$($summary.managedImportMethodCount) sha256=$($summary.mappingSha256)"
