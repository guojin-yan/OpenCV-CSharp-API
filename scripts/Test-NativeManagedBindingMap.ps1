...
function Test-BindingMapDocument {
    ...
    Assert-True -List $List -Condition ($bindingLines.Count -eq 2656 -and $actualEntrypoints.Count -eq 2656) -Path $Path -Issue "Binding-map native entry count drifted" -Text "actual=$($actualEntrypoints.Count) expected=2656"
...
Write-Host "NATIVE_MANAGED_BINDING_MAP_CONTRACT_OK native=2656 bound=2656 unbound=0 managed_only=0 imports=$($summary.managedImportMethodCount) sha256=$($summary.mappingSha256)"
