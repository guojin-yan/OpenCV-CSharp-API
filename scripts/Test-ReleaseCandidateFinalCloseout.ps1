...
function Test-Record {
    ...
    Assert-True -List $List -Condition ($androidBlocker.Count -eq 1 -and $androidBlocker[0].Status -eq "android-evidence-pending" -and $androidBlocker[0].Evidence -match "x64/x86 Full and Mini" -and $androidBlocker[0].Evidence -match "single-loader emulator revalidation" -and $androidBlocker[0].Evidence -match "ARM/ARM64" -and $androidBlocker[0].Evidence -match "device loading evidence") -Issue "Final closeout Android blocker must distinguish emulator revalidation from pending ARM device evidence"
}
