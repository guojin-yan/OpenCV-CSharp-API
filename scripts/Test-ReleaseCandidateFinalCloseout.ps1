...
function Test-Record {
    ...
    Assert-True -List $List -Condition ($Record.ApiAbiBaseline.Managed.Sha256 -eq "504b369eaf7138f0a42fd299782c61f613681e71450f9c3d3c443cdac555422a" -and $Record.ApiAbiBaseline.Managed.TypeCount -eq 558 -and $Record.ApiAbiBaseline.Managed.MemberCount -eq 6095 -and $Record.ApiAbiBaseline.Managed.NamespaceCount -eq 41 -and $Record.ApiAbiBaseline.Managed.TargetFramework -eq "net8.0") -Issue "Final closeout managed API baseline evidence drifted"
    Assert-True -List $List -Condition ($Record.ApiAbiBaseline.NativeFull.Sha256 -eq "e8de64559dce973b900a9fafb399e7f91d3423d40c6f76f89308d36b65287372" -and $Record.ApiAbiBaseline.NativeFull.FunctionCount -eq 2489 -and $Record.ApiAbiBaseline.NativeMini.Sha256 -eq "6101a6d4d71c3fce8baff6f5f2184962da95f44d8166b94bbbca47e4aa626395" -and $Record.ApiAbiBaseline.NativeMini.FunctionCount -eq 526) -Issue "Final closeout native ABI baseline evidence drifted"
    Assert-True -List $List -Condition ($Record.ApiAbiBaseline.NativeManagedBindingMap.Sha256 -eq "51acb78554e976d5c91096164746ffe1f97da2818a0ff7d2805a9c60f12adb2b" -and $Record.ApiAbiBaseline.NativeManagedBindingMap.NativeFunctionCount -eq 2489 -and $Record.ApiAbiBaseline.NativeManagedBindingMap.ManagedBoundCount -eq 2489 -and $Record.ApiAbiBaseline.NativeManagedBindingMap.UnboundCount -eq 0 -and $Record.ApiAbiBaseline.NativeManagedBindingMap.ManagedOnlyCount -eq 0) -Issue "Final closeout native-to-managed binding evidence drifted"
