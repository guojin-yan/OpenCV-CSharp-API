...
if ([string]::IsNullOrWhiteSpace($DotNetPath)) {
    ...
    if ($null -eq $dotnetCommand) {
        throw "dotnet was not found. Calib3D upstream-map generation requires a .NET 10 SDK."
    }
...
if ($LASTEXITCODE -ne 0 -or $sdkVersion -notmatch '^10\.') {
    throw "Calib3D upstream-map generation requires a .NET 10 SDK; resolved '$sdkVersion' from $resolvedDotNet."
}
