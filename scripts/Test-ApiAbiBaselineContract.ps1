...
function Test-GapInventory {
    ...
    Assert-True -List $List -Condition ($Inventory.baselines.nativeFull.sha256 -eq $NativeFullHash -and [int]$Inventory.baselines.nativeFull.functionCount -eq 2452 -and $Inventory.baselines.nativeFull.primaryPrefix -eq "jyppx_ocv_") -Path $Path -Issue "API gap inventory full native baseline evidence drifted"
...
function Test-MlFamilyInventory {
    ...
    Assert-True -List $List -Condition ($Inventory.schemaVersion -eq 1 -and $Inventory.status -eq "implemented-verified" -and $Inventory.upstreamOpenCvVersion -eq "5.0.0" -and [int]$Inventory.managedPublicTypeAdditionCount -eq 4 -and [int]$Inventory.managedPublicMemberAdditionCount -eq 39 -and [int]$Inventory.nativeEntrypointAdditionCount -eq 14 -and $families.Count -eq 1 -and $families[0].id -eq "ml-ann-mlp" -and $extensions.Count -eq 1) -Path $Path -Issue "ML family inventory identity, status, count, or order drifted"
    if ($families.Count -ne 1 -or $extensions.Count -ne 1) { return }
    ...
    foreach ($declaration in $families[0].declarations) {
...
Test-NativeManifestDocument -Text $nativeFullText -ExpectedCount 2452 -Mini $false -List $violations -Path "src/OpenCvSharp.Native/generated/legacy_abi_manifest.txt"
...
Assert-FixtureRejected -Name "reordered native ABI" -ExpectedIssue "ordinal function-name ordering" -Action {
    ...
    Test-NativeManifestDocument -Text (($fixture -join "`n") + "`n") -ExpectedCount 2452 -Mini $false -List $list -Path "fixture/reordered-native-abi.txt"
}
Assert-FixtureRejected -Name "fixed-major primary native ABI" -ExpectedIssue "primary/compatibility identity" -Action {
    ...
    Test-NativeManifestDocument -Text (($fixture -join "`n") + "`n") -ExpectedCount 2452 -Mini $false -List $list -Path "fixture/fixed-major-primary-native-abi.txt"
}
...
Write-Host "API_ABI_BASELINE_OK managed_types=$($managedSummary.typeCount) managed_members=$($managedSummary.memberCount) namespaces=$($managedSummary.namespaceCount) native_full=2452 native_mini=526 imgproc_callables=167 imgproc_implemented=161 imgcodecs_callables=22 imgcodecs_implemented=22 videoio_callables=40 videoio_implemented=40 registry_operations=12 calib3d_callables=167 calib3d_implemented=167 core_callables=215 core_implemented=176 dnn_callables=159 dnn_implemented=70 features_callables=160 features_implemented=134 objdetect_callables=163 objdetect_implemented=153 photo_callables=120 photo_implemented=120 ml_callables=208 ml_implemented=119 ml_missing=89 video_callables=145 video_implemented=138 video_missing=0 managed_sha256=$managedHash"
