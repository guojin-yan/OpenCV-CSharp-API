...
$highGuiMapPath = Join-Path $repo "compatibility/highgui-upstream-map.txt"
$highGuiSummaryPath = Join-Path $repo "compatibility/highgui-upstream-summary.json"
$highGuiFamilyPath = Join-Path $repo "compatibility/highgui-implemented-families.json"
$highGuiRawPath = Join-Path $repo "compatibility/highgui-upstream-raw.json"
...
$mlMapPath = Join-Path $repo "compatibility/ml-upstream-map.txt"
...
$stitchingMapPath = Join-Path $repo "compatibility/stitching-upstream-map.txt"
$stitchingSummaryPath = Join-Path $repo "compatibility/stitching-upstream-summary.json"
$stitchingRawPath = Join-Path $repo "compatibility/stitching-upstream-raw.json"
...
function Test-GapInventory {
    param(
        ...
        [Parameter(Mandatory)][object]$HighGuiSummary,
        [Parameter(Mandatory)][string]$HighGuiMapHash,
        [Parameter(Mandatory)][string]$HighGuiRawHash,
        ...
        [Parameter(Mandatory)][object]$StitchingSummary,
        [Parameter(Mandatory)][string]$StitchingMapHash,
        [Parameter(Mandatory)][string]$StitchingRawHash,
    ...
    Assert-True -List $List -Condition ($Inventory.baselines.nativeFull.sha256 -eq $NativeFullHash -and [int]$Inventory.baselines.nativeFull.functionCount -eq 2663 -and $Inventory.baselines.nativeFull.primaryPrefix -eq "jyppx_ocv_") -Path $Path -Issue "API gap inventory full native baseline evidence drifted"
    Assert-True -List $List -Condition ($Inventory.baselines.nativeMini.sha256 -eq $NativeMiniHash -and [int]$Inventory.baselines.nativeMini.functionCount -eq 527 -and $Inventory.baselines.nativeMini.primaryPrefix -eq "jyppx_ocv_") -Path $Path -Issue "API gap inventory mini native baseline evidence drifted"
    ...
    $highGuiInventoryMatches =
        $Inventory.baselines.highGuiUpstreamMap.sha256 -eq $HighGuiMapHash -and
        $Inventory.baselines.highGuiUpstreamMap.summaryPath -eq "compatibility/highgui-upstream-summary.json" -and
        $Inventory.baselines.highGuiUpstreamMap.classificationPath -eq "compatibility/highgui-upstream-classifications.json" -and
        $Inventory.baselines.highGuiUpstreamMap.familyInventoryPath -eq "compatibility/highgui-implemented-families.json" -and
        $Inventory.baselines.highGuiUpstreamMap.rawExtractionPath -eq "compatibility/highgui-upstream-raw.json" -and
        $HighGuiRawHash -eq (Get-FileHash -LiteralPath $highGuiRawPath -Algorithm SHA256).Hash.ToLowerInvariant() -and
        [int]$Inventory.baselines.highGuiUpstreamMap.declarationCount -eq [int]$HighGuiSummary.declarationCount -and
        [int]$Inventory.baselines.highGuiUpstreamMap.callableCount -eq [int]$HighGuiSummary.callableCount -and
        [int]$Inventory.baselines.highGuiUpstreamMap.implementedCount -eq [int]$HighGuiSummary.classificationCounts.implemented -and
        [int]$Inventory.baselines.highGuiUpstreamMap.missingCount -eq 0 -and
        [int]$Inventory.baselines.highGuiUpstreamMap.intentionallyOmittedCount -eq [int]$HighGuiSummary.classificationCounts.'intentionally-omitted' -and
        [int]$Inventory.baselines.highGuiUpstreamMap.unsupportedCount -eq [int]$HighGuiSummary.classificationCounts.unsupported -and
        [int]$Inventory.baselines.highGuiUpstreamMap.upstreamConditionalCount -eq [int]$HighGuiSummary.classificationCounts.'upstream-conditional' -and
        [int]$Inventory.baselines.highGuiUpstreamMap.sourceHeaderCount -eq [int]$HighGuiSummary.sourceHeaderCount -and
        [int]$Inventory.baselines.highGuiUpstreamMap.compatibilityHeaderCount -eq [int]$HighGuiSummary.compatibilityHeaderCount -and
        [int]$Inventory.baselines.highGuiUpstreamMap.excludedPublicHeaderCount -eq [int]$HighGuiSummary.excludedPublicHeaderCount -and
        [int]$Inventory.baselines.highGuiUpstreamMap.sourceReviewedExtensionCount -eq [int]$HighGuiSummary.sourceReviewedExtensionCount -and
        [int]$Inventory.baselines.highGuiUpstreamMap.selectedFamilyCount -eq [int]$HighGuiSummary.selectedFamilyCount -and
        [int]$Inventory.baselines.highGuiUpstreamMap.selectedDeclarationCount -eq [int]$HighGuiSummary.selectedDeclarationCount -and
        [int]$Inventory.baselines.highGuiUpstreamMap.managedPublicTypeAdditionCount -eq [int]$HighGuiSummary.managedPublicTypeAdditionCount -and
        [int]$Inventory.baselines.highGuiUpstreamMap.managedPublicMemberAdditionCount -eq [int]$HighGuiSummary.managedPublicMemberAdditionCount -and
        [int]$Inventory.baselines.highGuiUpstreamMap.nativeEntrypointAdditionCount -eq [int]$HighGuiSummary.nativeEntrypointAdditionCount
    Assert-True -List $List -Condition $highGuiInventoryMatches -Path $Path -Issue "API gap inventory HighGui upstream-map evidence drifted"
    ...
    $mlInventoryMatches =
        $Inventory.baselines.mlUpstreamMap.sha256 -eq $MlMapHash -and
        $Inventory.baselines.mlUpstreamMap.rawSha256 -eq $MlRawHash -and
        $Inventory.baselines.mlUpstreamMap.summaryPath -eq "compatibility/ml-upstream-summary.json" -and
        $Inventory.baselines.mlUpstreamMap.classificationPath -eq "compatibility/ml-upstream-classifications.json" -and
        $Inventory.baselines.mlUpstreamMap.familyInventoryPath -eq "compatibility/ml-implemented-families.json" -and
        $Inventory.baselines.mlUpstreamMap.rawExtractionPath -eq "compatibility/ml-upstream-raw.json" -and
        [int]$Inventory.baselines.mlUpstreamMap.declarationCount -eq [int]$MlSummary.declarationCount -and
        [int]$Inventory.baselines.mlUpstreamMap.callableCount -eq [int]$MlSummary.callableCount -and
        [int]$Inventory.baselines.mlUpstreamMap.implementedCount -eq [int]$MlSummary.classificationCounts.implemented -and
        [int]$Inventory.baselines.mlUpstreamMap.missingCount -eq [int]$MlSummary.classificationCounts.missing -and
        [int]$Inventory.baselines.mlUpstreamMap.intentionallyOmittedCount -eq [int]$MlSummary.classificationCounts.'intentionally-omitted' -and
        [int]$Inventory.baselines.mlUpstreamMap.unsupportedCount -eq [int]$MlSummary.classificationCounts.unsupported -and
        [int]$Inventory.baselines.mlUpstreamMap.upstreamConditionalCount -eq [int]$MlSummary.classificationCounts.'upstream-conditional' -and
        [int]$Inventory.baselines.mlUpstreamMap.sourceHeaderCount -eq [int]$MlSummary.sourceHeaderCount -and
        [int]$Inventory.baselines.mlUpstreamMap.compatibilityHeaderCount -eq [int]$MlSummary.compatibilityHeaderCount -and
        [int]$Inventory.baselines.mlUpstreamMap.excludedPublicHeaderCount -eq [int]$MlSummary.excludedPublicHeaderCount -and
        [int]$Inventory.baselines.mlUpstreamMap.selectedFamilyCount -eq [int]$MlSummary.selectedFamilyCount -and
        [int]$Inventory.baselines.mlUpstreamMap.selectedDeclarationCount -eq [int]$MlSummary.selectedDeclarationCount -and
        [int]$Inventory.baselines.mlUpstreamMap.sourceReviewedExtensionCount -eq [int]$MlSummary.sourceReviewedExtensionCount -and
        [int]$Inventory.baselines.mlUpstreamMap.managedPublicTypeAdditionCount -eq [int]$MlSummary.managedPublicTypeAdditionCount -and
    ...
    $stitchingInventoryMatches =
        $Inventory.baselines.stitchingUpstreamMap.sha256 -eq $StitchingMapHash -and
        $Inventory.baselines.stitchingUpstreamMap.rawSha256 -eq $StitchingRawHash -and
        $Inventory.baselines.stitchingUpstreamMap.summaryPath -eq "compatibility/stitching-upstream-summary.json" -and
        $Inventory.baselines.stitchingUpstreamMap.classificationPath -eq "compatibility/stitching-upstream-classifications.json" -and
        $Inventory.baselines.stitchingUpstreamMap.familyInventoryPath -eq "compatibility/stitching-implemented-families.json" -and
        $Inventory.baselines.stitchingUpstreamMap.rawExtractionPath -eq "compatibility/stitching-upstream-raw.json" -and
        [int]$Inventory.baselines.stitchingUpstreamMap.declarationCount -eq [int]$StitchingSummary.declarationCount -and
        [int]$Inventory.baselines.stitchingUpstreamMap.callableCount -eq [int]$StitchingSummary.callableCount -and
        [int]$Inventory.baselines.stitchingUpstreamMap.implementedCount -eq [int]$StitchingSummary.classificationCounts.implemented -and
        [int]$Inventory.baselines.stitchingUpstreamMap.missingCount -eq [int]$StitchingSummary.classificationCounts.missing -and
        [int]$Inventory.baselines.stitchingUpstreamMap.intentionallyOmittedCount -eq [int]$StitchingSummary.classificationCounts.'intentionally-omitted' -and
        [int]$Inventory.baselines.stitchingUpstreamMap.unsupportedCount -eq [int]$StitchingSummary.classificationCounts.unsupported -and
        [int]$Inventory.baselines.stitchingUpstreamMap.upstreamConditionalCount -eq [int]$StitchingSummary.classificationCounts.'upstream-conditional' -and
        [int]$Inventory.baselines.stitchingUpstreamMap.highLevelDeclarationCount -eq [int]$StitchingSummary.surfaceCounts.primary.declarations -and
        [int]$Inventory.baselines.stitchingUpstreamMap.highLevelCallableCount -eq [int]$StitchingSummary.surfaceCounts.primary.callables -and
        [int]$Inventory.baselines.stitchingUpstreamMap.highLevelImplementedCount -eq [int]$StitchingSummary.surfaceCounts.primary.implemented -and
        [int]$Inventory.baselines.stitchingUpstreamMap.publicWarperDeclarationCount -eq [int]$StitchingSummary.surfaceCounts.'public-warpers'.declarations -and
        [int]$Inventory.baselines.stitchingUpstreamMap.publicWarperCallableCount -eq [int]$StitchingSummary.surfaceCounts.'public-warpers'.callables -and
        [int]$Inventory.baselines.stitchingUpstreamMap.publicWarperImplementedCount -eq [int]$StitchingSummary.surfaceCounts.'public-warpers'.implemented -and
        [int]$Inventory.baselines.stitchingUpstreamMap.blenderDeclarationCount -eq [int]$StitchingSummary.surfaceCounts.'detail-blenders'.declarations -and
        [int]$Inventory.baselines.stitchingUpstreamMap.blenderCallableCount -eq [int]$StitchingSummary.surfaceCounts.'detail-blenders'.callables -and
        [int]$Inventory.baselines.stitchingUpstreamMap.blenderImplementedCount -eq [int]$StitchingSummary.surfaceCounts.'detail-blenders'.implemented -and
        [int]$Inventory.baselines.stitchingUpstreamMap.exposureDeclarationCount -eq [int]$StitchingSummary.surfaceCounts.'detail-exposure'.declarations -and
        [int]$Inventory.baselines.stitchingUpstreamMap.exposureCallableCount -eq [int]$StitchingSummary.surfaceCounts.'detail-exposure'.callables -and
        [int]$Inventory.baselines.stitchingUpstreamMap.exposureImplementedCount -eq [int]$StitchingSummary.surfaceCounts.'detail-exposure'.implemented -and
        [int]$Inventory.baselines.stitchingUpstreamMap.matcherDeclarationCount -eq [int]$StitchingSummary.surfaceCounts.'detail-matchers'.declarations -and
        [int]$Inventory.baselines.stitchingUpstreamMap.matcherCallableCount -eq [int]$StitchingSummary.surfaceCounts.'detail-matchers'.callables -and
        [int]$Inventory.baselines.stitchingUpstreamMap.matcherImplementedCount -eq [int]$StitchingSummary.surfaceCounts.'detail-matchers'.implemented -and
        [int]$Inventory.baselines.stitchingUpstreamMap.seamFinderDeclarationCount -eq [int]$StitchingSummary.surfaceCounts.'detail-seam-finders'.declarations -and
        [int]$Inventory.baselines.stitchingUpstreamMap.seamFinderCallableCount -eq [int]$StitchingSummary.surfaceCounts.'detail-seam-finders'.callables -and
        [int]$Inventory.baselines.stitchingUpstreamMap.seamFinderImplementedCount -eq [int]$StitchingSummary.surfaceCounts.'detail-seam-finders'.implemented -and
        [int]$Inventory.baselines.stitchingUpstreamMap.timelapserDeclarationCount -eq [int]$StitchingSummary.surfaceCounts.'detail-timelapsers'.declarations -and
        [int]$Inventory.baselines.stitchingUpstreamMap.timelapserCallableCount -eq [int]$StitchingSummary.surfaceCounts.'detail-timelapsers'.callables -and
        [int]$Inventory.baselines.stitchingUpstreamMap.timelapserImplementedCount -eq [int]$StitchingSummary.surfaceCounts.'detail-timelapsers'.implemented -and
        [int]$Inventory.baselines.stitchingUpstreamMap.utilityDeclarationCount -eq [int]$StitchingSummary.surfaceCounts.'detail-util'.declarations -and
        [int]$Inventory.baselines.stitchingUpstreamMap.utilityCallableCount -eq [int]$StitchingSummary.surfaceCounts.'detail-util'.callables -and
        [int]$Inventory.baselines.stitchingUpstreamMap.utilityImplementedCount -eq [int]$StitchingSummary.surfaceCounts.'detail-util'.implemented -and
        [int]$Inventory.baselines.stitchingUpstreamMap.detailWarperDeclarationCount -eq [int]$StitchingSummary.surfaceCounts.'detail-warpers'.declarations -and
        [int]$Inventory.baselines.stitchingUpstreamMap.detailWarperCallableCount -eq [int]$StitchingSummary.surfaceCounts.'detail-warpers'.callables -and
        [int]$Inventory.baselines.stitchingUpstreamMap.detailWarperImplementedCount -eq [int]$StitchingSummary.surfaceCounts.'detail-warpers'.implemented -and
        [int]$Inventory.baselines.stitchingUpstreamMap.sourceHeaderCount -eq [int]$StitchingSummary.sourceHeaderCount -and
        [int]$Inventory.baselines.stitchingUpstreamMap.compatibilityHeaderCount -eq [int]$StitchingSummary.compatibilityHeaderCount -and
        [int]$Inventory.baselines.stitchingUpstreamMap.selectedFamilyCount -eq [int]$StitchingSummary.selectedFamilyCount -and
        [int]$Inventory.baselines.stitchingUpstreamMap.selectedDeclarationCount -eq [int]$StitchingSummary.selectedDeclarationCount -and
        [int]$Inventory.baselines.stitchingUpstreamMap.sourceReviewedExtensionCount -eq [int]$StitchingSummary.sourceReviewedExtensionCount -and
        [int]$Inventory.baselines.stitchingUpstreamMap.managedPublicTypeAdditionCount -eq [int]$StitchingSummary.managedPublicTypeAdditionCount -and
        [int]$Inventory.baselines.stitchingUpstreamMap.managedPublicMemberAdditionCount -eq [int]$StitchingSummary.managedPublicMemberAdditionCount -and
        [int]$Inventory.baselines.stitchingUpstreamMap.nativeEntrypointAdditionCount -eq [int]$StitchingSummary.nativeEntrypointAdditionCount
    Assert-True -List $List -Condition $stitchingInventoryMatches -Path $Path -Issue "API gap inventory Stitching upstream-map evidence drifted"
...
function Test-MlFamilyInventory {
    param(
        [Parameter(Mandatory)][object]$Summary,
    ...
    Assert-True -List $List -Condition ($Summary.mappingSha256 -eq $MappingHash -and $Summary.familyInventorySha256 -eq $InventoryHash -and [int]$Summary.negativeFixtureCount -eq 17) -Path $Path -Issue "ML upstream mapping, inventory, or fixture evidence drifted"
    $families = @($Inventory.families)
    $extensions = @($Inventory.sourceReviewedExtensions)
    Assert-True -List $List -Condition ($Inventory.schemaVersion -eq 1 -and $Inventory.status -eq "implemented-verified" -and $Inventory.upstreamOpenCvVersion -eq "5.0.0" -and [int]$Inventory.managedPublicTypeAdditionCount -eq 18 -and [int]$Inventory.managedPublicMemberAdditionCount -eq 147 -and [int]$Inventory.nativeEntrypointAdditionCount -eq 75 -and $families.Count -eq 6 -and $families[0].id -eq "ml-ann-mlp" -and $families[1].id -eq "ml-em" -and $families[2].id -eq "ml-tree-models" -and $families[3].id -eq "ml-train-data-buffers" -and $families[4].id -eq "ml-logistic-regression" -and $families[5].id -eq "ml-svmsgd" -and $extensions.Count -eq 2) -Path $Path -Issue "ML family inventory identity, status, count, or order drifted"
    if ($families.Count -ne 6 -or $extensions.Count -ne 2) { return }

    Assert-True -List $List -Condition (@($families[0].declarations).Count -eq 32 -and -not [string]::IsNullOrWhiteSpace([string]$families[0].rationale)) -Path $Path -Issue "ML ANN_MLP declaration count or rationale drifted"
    Assert-True -List $List -Condition (@($families[1].declarations).Count -eq 16 -and -not [string]::IsNullOrWhiteSpace([string]$families[1].rationale)) -Path $Path -Issue "ML EM declaration count or rationale drifted"
    Assert-True -List $List -Condition (@($families[2].declarations).Count -eq 38 -and -not [string]::IsNullOrWhiteSpace([string]$families[2].rationale)) -Path $Path -Issue "ML tree-model declaration count or rationale drifted"
    Assert-True -List $List -Condition (@($families[3].declarations).Count -eq 2 -and -not [string]::IsNullOrWhiteSpace([string]$families[3].rationale)) -Path $Path -Issue "ML TrainData-buffer declaration count or rationale drifted"
    Assert-True -List $List -Condition (@($families[4].declarations).Count -eq 16 -and -not [string]::IsNullOrWhiteSpace([string]$families[4].rationale)) -Path $Path -Issue "ML LogisticRegression declaration count or rationale drifted"
    Assert-True -List $List -Condition (@($families[5].declarations).Count -eq 17 -and -not [string]::IsNullOrWhiteSpace([string]$families[5].rationale)) -Path $Path -Issue "ML SVMSGD declaration count or rationale drifted"
    foreach ($declaration in @($families | ForEach-Object { $_.declarations })) {
        Assert-True -List $List -Condition (@($declaration.nativeEntrypoints).Count -gt 0 -and @($declaration.managedMembers).Count -gt 0) -Path $Path -Issue "ML ANN_MLP declaration is missing native or managed evidence" -Text $declaration.upstreamIdentity
        Assert-True -List $List -Condition ($declaration.focusedTest -eq "tests/OpenCvSharp.Tests/ML/MLTests.cs" -and $declaration.nativeSmoke -eq "src/OpenCvSharp.Native/tests/native_smoke.cpp" -and $declaration.sample -eq "samples/ConsoleSamples/Program.cs" -and $declaration.guide -eq "docs/articles/ml-guide.md") -Path $Path -Issue "ML ANN_MLP test/sample/guide evidence drifted" -Text $declaration.upstreamIdentity
    }
    Assert-True -List $List -Condition ($extensions[0].upstreamIdentity -eq "cv.ml.ANN_MLP.setAnnealEnergyRNG(const RNG& rng)->void" -and @($extensions[0].nativeEntrypoints).Count -eq 1 -and @($extensions[0].managedMembers).Count -eq 1 -and [string]$extensions[0].adaptation -match "excluded from parser-derived counts") -Path $Path -Issue "ML source-reviewed RNG adaptation drifted"
...
$highGuiGuardArguments = @{ RepositoryRoot = $repo }
if (-not [string]::IsNullOrWhiteSpace($DotNetPath)) { $highGuiGuardArguments.DotNetPath = $DotNetPath }
& (Join-Path $repo "scripts/Test-HighGuiUpstreamMap.ps1") @highGuiGuardArguments
if (-not $?) { throw "HighGui upstream-map generated-file freshness check failed." }
...
$mlGuardArguments = @{ RepositoryRoot = $repo }
if (-not [string]::IsNullOrWhiteSpace($DotNetPath)) { $mlGuardArguments.DotNetPath = $DotNetPath }
& (Join-Path $repo "scripts/Test-MlUpstreamMap.ps1") @mlGuardArguments
if (-not $?) { throw "ML upstream-map generated-file freshness check failed." }
$trackingGuardArguments = @{ RepositoryRoot = $repo }
if (-not [string]::IsNullOrWhiteSpace($DotNetPath)) { $trackingGuardArguments.DotNetPath = $DotNetPath }
& (Join-Path $repo "scripts/Test-TrackingUpstreamMap.ps1") @trackingGuardArguments
if (-not $?) { throw "Tracking upstream-map generated-file freshness check failed." }
$stitchingGuardArguments = @{ RepositoryRoot = $repo }
if (-not [string]::IsNullOrWhiteSpace($DotNetPath)) { $stitchingGuardArguments.DotNetPath = $DotNetPath }
& (Join-Path $repo "scripts/Test-StitchingUpstreamMap.ps1") @stitchingGuardArguments
if (-not $?) { throw "Stitching upstream-map generated-file freshness check failed." }
...
$highGuiMapText = [IO.File]::ReadAllText($highGuiMapPath)
$highGuiSummary = Get-Content -LiteralPath $highGuiSummaryPath -Raw | ConvertFrom-Json
...
$mlMapText = [IO.File]::ReadAllText($mlMapPath)
...
$stitchingMapText = [IO.File]::ReadAllText($stitchingMapPath)
$stitchingSummary = Get-Content -LiteralPath $stitchingSummaryPath -Raw | ConvertFrom-Json
...
$highGuiMapHash = Get-TextSha256 $highGuiMapText
$highGuiRawHash = (Get-FileHash -LiteralPath $highGuiRawPath -Algorithm SHA256).Hash.ToLowerInvariant()
...
$stitchingMapHash = Get-TextSha256 $stitchingMapText
$stitchingRawHash = (Get-FileHash -LiteralPath $stitchingRawPath -Algorithm SHA256).Hash.ToLowerInvariant()
...
Test-NativeManifestDocument -Text $nativeFullText -ExpectedCount 2663 -Mini $false -List $violations -Path "src/OpenCvSharp.Native/generated/native_abi_manifest.txt"
Test-NativeManifestDocument -Text $nativeMiniText -ExpectedCount 527 -Mini $true -List $violations -Path "src/OpenCvSharp.Native/generated/native_abi_mini_manifest.txt"
...
Test-MlFamilyInventory -Summary $mlSummary -Inventory $mlFamily -MappingHash $mlMapHash -InventoryHash $mlFamilyHash -List $violations -Path "compatibility/ml-implemented-families.json"
...
Assert-FixtureRejected -Name "reordered native ABI" -ExpectedIssue "ordinal function-name ordering" -Action {
    ...
    Test-NativeManifestDocument -Text (($fixture -join "`n") + "`n") -ExpectedCount 2663 -Mini $false -List $list -Path "fixture/reordered-native-abi.txt"
}
Assert-FixtureRejected -Name "fixed-major primary native ABI" -ExpectedIssue "version-neutral identity" -Action {
    ...
    Test-NativeManifestDocument -Text (($fixture -join "`n") + "`n") -ExpectedCount 2663 -Mini $false -List $list -Path "fixture/fixed-major-primary-native-abi.txt"
}
Assert-FixtureRejected -Name "missing gap inventory" -ExpectedIssue "gap inventory is missing" -Action {
    param($list)
    Test-GapInventory -Inventory $null -Summary $managedSummary -ManagedHash $managedHash -NativeFullHash $nativeFullHash -NativeMiniHash $nativeMiniHash -BindingSummary $bindingSummary -BindingMapHash $bindingMapHash -ImgProcSummary $imgProcSummary -ImgProcMapHash $imgProcMapHash -ImgCodecsSummary $imgCodecsSummary -ImgCodecsMapHash $imgCodecsMapHash -VideoIOSummary $videoIOSummary -VideoIOMapHash $videoIOMapHash -VideoIORegistryHash $videoIORegistryHash -Calib3DSummary $calib3DSummary -Calib3DMapHash $calib3DMapHash -CoreSummary $coreSummary -CoreMapHash $coreMapHash -DnnSummary $dnnSummary -DnnMapHash $dnnMapHash -FeaturesSummary $featuresSummary -FeaturesMapHash $featuresMapHash -HighGuiSummary $highGuiSummary -HighGuiMapHash $highGuiMapHash -HighGuiRawHash $highGuiRawHash -ObjDetectSummary $objDetectSummary -ObjDetectMapHash $objDetectMapHash -ObjDetectRawHash $objDetectRawHash -PhotoSummary $photoSummary -PhotoMapHash $photoMapHash -PhotoRawHash $photoRawHash -MlSummary $mlSummary -MlMapHash $mlMapHash -MlRawHash $mlRawHash -TrackingSummary $trackingSummary -TrackingMapHash $trackingMapHash -TrackingRawHash $trackingRawHash -StitchingSummary $stitchingSummary -StitchingMapHash $stitchingMapHash -StitchingRawHash $stitchingRawHash -VideoSummary $videoSummary -VideoMapHash $videoMapHash -VideoRawHash $videoRawHash -List $list -Path "fixture/missing-gap-inventory.json"
}
...
Write-Host "API_ABI_BASELINE_OK managed_types=$($managedSummary.typeCount) managed_members=$($managedSummary.memberCount) namespaces=$($managedSummary.namespaceCount) native_full=2663 native_mini=527 imgproc_callables=167 imgproc_implemented=161 imgcodecs_callables=22 imgcodecs_implemented=22 videoio_callables=40 videoio_implemented=40 registry_operations=12 calib3d_callables=167 calib3d_implemented=167 core_callables=215 core_implemented=176 dnn_callables=159 dnn_implemented=74 features_callables=160 features_implemented=134 highgui_callables=26 highgui_implemented=20 highgui_missing=0 objdetect_callables=163 objdetect_implemented=153 photo_callables=120 photo_implemented=120 ml_callables=208 ml_implemented=208 ml_missing=0 tracking_callables=21 tracking_implemented=21 tracking_missing=0 stitching_callables=158 stitching_implemented=156 stitching_missing=0 stitching_unsupported=2 video_callables=145 video_implemented=138 video_missing=0 managed_sha256=$managedHash"
