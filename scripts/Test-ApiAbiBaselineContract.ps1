...
function Test-GapInventory {
    ...
    Assert-True -List $List -Condition ($Inventory.baselines.nativeFull.sha256 -eq $NativeFullHash -and [int]$Inventory.baselines.nativeFull.functionCount -eq 2603 -and $Inventory.baselines.nativeFull.primaryPrefix -eq "jyppx_ocv_") -Path $Path -Issue "API gap inventory full native baseline evidence drifted"
...
Test-NativeManifestDocument -Text $nativeFullText -ExpectedCount 2603 -Mini $false -List $violations -Path "src/OpenCvSharp.Native/generated/legacy_abi_manifest.txt"
...
Assert-FixtureRejected -Name "reordered native ABI" -ExpectedIssue "ordinal function-name ordering" -Action {
    ...
    Test-NativeManifestDocument -Text (($fixture -join "`n") + "`n") -ExpectedCount 2603 -Mini $false -List $list -Path "fixture/reordered-native-abi.txt"
}
Assert-FixtureRejected -Name "fixed-major primary native ABI" -ExpectedIssue "primary/compatibility identity" -Action {
    ...
    Test-NativeManifestDocument -Text (($fixture -join "`n") + "`n") -ExpectedCount 2603 -Mini $false -List $list -Path "fixture/fixed-major-primary-native-abi.txt"
}
