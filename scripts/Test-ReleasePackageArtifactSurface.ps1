...
Assert-Contains -Violations $violations -Path $bindingMapGuardPath -Text $bindingMapGuardText -Needle 'native=2513 bound=2513 unbound=0 managed_only=0' -Issue "Native-to-managed binding guard must emit complete parity evidence"
...
Assert-Contains -Violations $violations -Path $bindingMapSummaryPath -Text $bindingMapSummaryText -Needle '"managedBoundCount": 2513' -Issue "Binding-map summary must record all native functions as managed-bound"
