...
Assert-Contains -Violations $violations -Path $publishNugetWorkflowPath -Text $publishNugetWorkflowText -Needle 'single_maintainer_exception' -Issue "First-preview publication must expose the explicit single-maintainer exception input"
Assert-Contains -Violations $violations -Path $publishNugetWorkflowPath -Text $publishNugetWorkflowText -Needle 'single-maintainer-first-preview-exception' -Issue "First-preview publication must record its single-maintainer approval mode"
Assert-Contains -Violations $violations -Path $publishNugetWorkflowPath -Text $publishNugetWorkflowText -Needle 'explicit-owner-authorization-no-independent-reviewer-available' -Issue "First-preview publication must record explicit owner risk acceptance"
Assert-Contains -Violations $violations -Path $publishNugetWorkflowPath -Text $publishNugetWorkflowText -Needle 'The single-maintainer exception is restricted to 5.0.0-preview.1.' -Issue "Single-maintainer exception must be version-bounded"
...
Assert-Contains -Violations $violations -Path $publicFeedVerificationContractPath -Text $publicFeedVerificationContractText -Needle "`$packageVersion = '5.0.0-preview.1'" -Issue "Public feed contract must verify the exact first-preview candidate version"
