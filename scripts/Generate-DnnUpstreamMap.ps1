...
if ($RegenerateRaw) {
    ...
    & $python (Join-Path $repo "tools/DnnUpstreamMap/extract_dnn.py") `
        --workspace $workspace `
        --opencv-root $opencv `
        --output $raw
    if ($LASTEXITCODE -ne 0) {
        throw "DNN raw extraction failed with exit code $LASTEXITCODE."
    }
}
if (-not (Test-Path -LiteralPath $raw -PathType Leaf)) {
    throw "Checked DNN raw artifact is missing: $raw"
}

$arguments = @(
    "run",
    "--project", (Join-Path $repo "tools/DnnUpstreamMap/DnnUpstreamMap.csproj"),
    ...
    "--family-output", (Join-Path $repo "compatibility/dnn-implemented-families.json")
)
...
try {
    ...
    & $dotnet @arguments
    if ($LASTEXITCODE -ne 0) {
        throw "DNN map generator failed with exit code $LASTEXITCODE."
    }
}
finally {
