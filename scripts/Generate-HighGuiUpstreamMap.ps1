...
if ($RegenerateRaw) {
    ...
    & $python (Join-Path $repo "tools/HighGuiUpstreamMap/extract_highgui.py") --workspace $workspace --opencv-root $opencv --output $raw
    if ($LASTEXITCODE -ne 0) { throw "HighGui raw extraction failed with exit code $LASTEXITCODE." }
}
if (-not (Test-Path -LiteralPath $raw -PathType Leaf)) { throw "Checked HighGui raw artifact is missing: $raw" }

$arguments = @(
    "run", "--project", (Join-Path $repo "tools/HighGuiUpstreamMap/HighGuiUpstreamMap.csproj"),
    ...
    "--family-output", (Join-Path $repo "compatibility/highgui-implemented-families.json")
)
...
try {
    ...
    $env:DOTNET_NOLOGO = "1"
    & $dotnet @arguments
    if ($LASTEXITCODE -ne 0) { throw "HighGui map generator failed with exit code $LASTEXITCODE." }
}
finally {
