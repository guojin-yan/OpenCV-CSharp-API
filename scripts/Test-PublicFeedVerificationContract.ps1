...
try {
    ...
    if (Test-Path -LiteralPath $publishWorkflowPath -PathType Leaf) {
        ...
        foreach ($token in @('https://api.nuget.org/v3/index.json','https://nuget.pkg.github.com/guojin-yan/index.json','environment: nuget-production','publish_authorization','verify_publication','secrets.NUGET_API_KEY','GITHUB_PACKAGES_TOKEN: ${{ github.token }}','scripts/Test-NuGetRepositorySignedPackage.ps1','github-packages-publication-proof.json','RequiredVisibility = ''public''','github.repository == ''guojin-yan/OpenCV-CSharp-API''')) {
