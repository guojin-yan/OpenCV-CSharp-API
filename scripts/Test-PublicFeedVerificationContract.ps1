...
try {
    ...
    if ($LiveFeedVerification) {
        ...
        Write-Host "NUGET_PUBLIC_FEED_READ_ONLY_OK index_status=$indexStatus package_status=$packageStatus package=$packageId/$packageVersion upload_attempted=false"
    }
    else {
