...
try {
    ...
    if (Test-Path -LiteralPath $publishWorkflowPath -PathType Leaf) {
        ...
        foreach ($token in @('https://api.nuget.org/v3/index.json','https://nuget.pkg.github.com/guojin-yan/index.json','environment: nuget-production','publish_authorization','single_maintainer_exception','single-maintainer-stable-5.0.0-exception','explicit-owner-authorized-stable-5.0.0-no-independent-reviewer','verify_publication','secrets.NUGET_API_KEY','GITHUB_PACKAGES_TOKEN: ${{ github.token }}','scripts/Test-NuGetRepositorySignedPackage.ps1','github-packages-publication-proof.json','RequiredVisibility = ''public''','github.repository == ''guojin-yan/OpenCV-CSharp-API''')) {
        ...
        $releaseTagBindingCount = [regex]::Matches($publishText, [regex]::Escape('RELEASE_TAG: ${{ inputs.release_tag }}')).Count
        Assert-True -List $violations -Condition ($releaseTagBindingCount -eq 2) -Path '.github/workflows/publish-nuget.yml' -Issue 'Stable release-tag input must be bound in both validation and release steps' -Text "actual=$releaseTagBindingCount expected=2"
