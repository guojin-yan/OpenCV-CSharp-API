...
$windowsX86JobText = Get-WorkflowJobText -JobName "verify-targeted-real-windows-x86"
...
foreach ($expectedTarget in @(
        ...
        [pscustomobject]@{ Rid = "win-x86"; Runner = "windows-latest" },
...
foreach ($expectation in @(
        ...
        @($workflowPath, $windowsX86JobText, "verify-targeted-real-windows-x86:", "Pack workflow must keep a separate Windows x86 verification job"),
        @($workflowPath, $windowsX86JobText, "inputs.rid == 'win-x86' && inputs.runtime_profile == 'full' && inputs.validate_synthetic_runtime != 'true' && inputs.publish_github_packages != 'true'", "Windows x86 verification must use the exact full-only non-synthetic non-publishing gate"),
        @($workflowPath, $windowsX86JobText, "runs-on: windows-latest", "Windows x86 verification must use the audited AMD64 Windows WoW64 host"),
        @($workflowPath, $windowsX86JobText, "PROCESSOR_ARCHITECTURE -ne 'AMD64'", "Windows x86 verification must require an AMD64 host"),
        @($workflowPath, $windowsX86JobText, "-Architecture x86", "Windows x86 verification must install an explicit x86 .NET runtime"),
        @($workflowPath, $windowsX86JobText, "raw.githubusercontent.com/dotnet/install-scripts/9c8552ae791982d3674fbb4d6ad887e536e7f506/src/dotnet-install.ps1", "Windows x86 verification must download the commit-pinned official .NET installer"),
        @($workflowPath, $windowsX86JobText, "BB1CE92F4397E24D4736A4658B9728FB8F9DB64A0D3F8E636BA408A866A6661D", "Windows x86 verification must require the audited installer SHA256"),
        @($workflowPath, $windowsX86JobText, "`$installScriptSha256.Equals(`$expectedInstallScriptSha256, [System.StringComparison]::OrdinalIgnoreCase)", "Windows x86 verification must compare the installer hash before execution"),
        @($workflowPath, $windowsX86JobText, "0x014c", "Windows x86 verification must inspect the x86 dotnet host as I386"),
        @($workflowPath, $windowsX86JobText, "WINDOWS_X86_DOTNET_HOST_OK", "Windows x86 verification must emit factual x86 .NET host evidence"),
        @($workflowPath, $windowsX86JobText, "name: nupkg-managed", "Windows x86 verification must download the same-run managed package"),
        @($workflowPath, $windowsX86JobText, "name: nupkg-win-x86-full", "Windows x86 verification must download only the exact full runtime artifact"),
        @($workflowPath, $windowsX86JobText, "-SelectedRid win-x86", "Windows x86 guards must select exact win-x86"),
        @($workflowPath, $windowsX86JobText, "-SelectedRuntimeProfile full", "Windows x86 guards must select only full"),
        @($workflowPath, $windowsX86JobText, "Test-WindowsRuntimePeClosure.ps1", "Windows x86 package must pass the reusable I386 PE closure audit"),
        ...
        @($workflowPath, $workflowText, "WINDOWS_X86_RUNTIME_INPUT_PROVENANCE_OK profile=full files=18 modules=17 sources=49 abi_functions=2663 synthetic=false machine=I386 wow64=true", "Pack runtime job must validate exact Windows x86 full provenance"),
        ...
        @($workflowPath, $workflowText, 'UBUNTU_22_04_X64_RUNTIME_INPUT_PROVENANCE_OK profile=mini files=$expectedPayloadFileCount modules=$expectedModuleCount sources=9 abi_functions=527 synthetic=false', "Pack runtime job must validate exact Ubuntu 22.04 x64 mini producer provenance"),
        ...
        @($workflowPath, $ubuntuArm64JobText, "expected_runtime_file_count=19", "Ubuntu ARM64 mini package audit must require exactly 19 runtime files"),
        @($workflowPath, $ubuntuArm64JobText, "expected_canonical_count=7", "Ubuntu ARM64 mini package audit must require exactly seven canonical ELFs"),
        ...
        @($workflowPath, $ubuntu2204Arm64JobText, "raw.githubusercontent.com/dotnet/install-scripts/9c8552ae791982d3674fbb4d6ad887e536e7f506/src/dotnet-install.sh", "Ubuntu 22.04 ARM64 verification must download the commit-pinned official .NET installer"),
        @($workflowPath, $ubuntu2204Arm64JobText, "082F7685E156738A1B2E2ED8381A621870D4CE8E8C59278034556F05C186EB2E", "Ubuntu 22.04 ARM64 verification must require the audited installer SHA256"),
        @($workflowPath, $ubuntu2204Arm64JobText, 'echo "$dotnet_install_script_sha256  /tmp/dotnet-install.sh" | sha256sum -c -', "Ubuntu 22.04 ARM64 verification must verify the installer payload before execution"),
        @($workflowPath, $ubuntu2204Arm64JobText, "--version 8.0.423 --architecture arm64", "Ubuntu 22.04 ARM64 verification must install the exact native ARM64 .NET 8 SDK"),
        ...
        @($workflowPath, $ubuntu2204Arm64JobText, "expected_runtime_file_count=19", "Ubuntu 22.04 ARM64 mini package audit must require exactly 19 runtime files"),
        @($workflowPath, $ubuntu2204Arm64JobText, "expected_canonical_count=7", "Ubuntu 22.04 ARM64 mini package audit must require exactly seven canonical ELFs"),
        ...
        @($workflowPath, $debianJobText, "container: debian:12@sha256:9344f8b8992482f80cba753f323adeaf17690076c095ccff6cc9536be98185dc", "Debian verification must execute in the audited Debian 12 job container"),
        @($workflowPath, $debianJobText, "2fa9c05d591a1582a9aba276272478c262e95ad00acf60eaee1644d93941e3c6", "Debian verifier must bind the audited Microsoft signing-key payload hash"),
        @($workflowPath, $debianJobText, "BC528686B50D79E339D3721CEB3E94ADBE1229CF", "Debian verifier must bind the audited full Microsoft signing-key fingerprint"),
        @($workflowPath, $debianJobText, "MICROSOFT_APT_TRUST_EVIDENCE rid=debian.12-x64", "Debian verifier must emit scoped verified APT trust evidence"),
        @($workflowPath, $debianJobText, "POWERSHELL_DEBIAN_PACKAGE_VERSION: 7.6.4-1.deb", "Debian verifier must bind the audited exact PowerShell Debian package version"),
        @($workflowPath, $debianJobText, "POWERSHELL_PACKAGE_SEMANTIC_VERSION: 7.6.4", "Debian verifier must bind the audited PowerShell semantic version"),
        @($workflowPath, $debianJobText, "POWERSHELL_APT_TOOLCHAIN_EVIDENCE rid=debian.12-x64", "Debian verifier must emit exact package-installed PowerShell evidence"),
        ...
        @($workflowPath, $debianJobText, "apt-get install -y --no-install-recommends `"powershell=`$POWERSHELL_DEBIAN_PACKAGE_VERSION`"", "Debian verification must install the exact audited PowerShell package before invoking repository guards"),
        @($workflowPath, $debianJobText, "test `"`$installed_powershell_package_architecture`" = `"amd64`"", "Debian verification must check the installed PowerShell package architecture"),
        @($workflowPath, $debianJobText, "test `"`$installed_powershell_version`" = `"`$POWERSHELL_PACKAGE_SEMANTIC_VERSION`"", "Debian verification must check the actual pwsh semantic version"),
        @($workflowPath, $debianJobText, "10.0.x", "Debian verification must install a .NET 10 SDK from the 10.0 feature band"),
        @($workflowPath, $debianJobText, "9.0.316", "Debian verification must install exact .NET 9 SDK 9.0.316"),
        @($workflowPath, $debianJobText, "8.0.423", "Debian verification must install exact .NET 8 SDK 8.0.423"),
        ...
        @($workflowPath, $debianJobText, "expected_runtime_file_count=19", "Debian mini package audit must require exactly 19 runtime files"),
        @($workflowPath, $debianJobText, "expected_canonical_count=7", "Debian mini package audit must require exactly seven canonical ELFs"),
        ...
        @($workflowPath, $workflowText, 'DEBIAN_12_X64_RUNTIME_INPUT_PROVENANCE_OK profile=mini files=$expectedPayloadFileCount modules=$expectedModuleCount sources=9 abi_functions=527 synthetic=false', "Pack runtime job must validate the exact Debian 12 x64 mini producer provenance"),
        ...
        @($workflowPath, $debianArm64JobText, "raw.githubusercontent.com/dotnet/install-scripts/9c8552ae791982d3674fbb4d6ad887e536e7f506/src/dotnet-install.sh", "Debian ARM64 verification must download the commit-pinned official .NET installer"),
        @($workflowPath, $debianArm64JobText, "082F7685E156738A1B2E2ED8381A621870D4CE8E8C59278034556F05C186EB2E", "Debian ARM64 verification must require the audited installer SHA256"),
        @($workflowPath, $debianArm64JobText, 'echo "$dotnet_install_script_sha256  /tmp/dotnet-install.sh" | sha256sum -c -', "Debian ARM64 verification must verify the installer payload before execution"),
        @($workflowPath, $debianArm64JobText, "--version 8.0.423 --architecture arm64", "Debian ARM64 verification must install the exact native ARM64 .NET 8 SDK"),
        ...
        @($workflowPath, $debianArm64JobText, "expected_runtime_file_count=19", "Debian ARM64 mini package audit must require exactly 19 runtime files"),
        @($workflowPath, $debianArm64JobText, "expected_canonical_count=7", "Debian ARM64 mini package audit must require exactly seven canonical ELFs"),
        ...
        @($workflowPath, $fedoraJobText, "container: fedora:40@sha256:3c86d25fef9d2001712bc3d9b091fc40cf04be4767e48f1aa3b785bf58d300ed", "Fedora verification must execute in the audited Fedora 40 job container"),
        @($workflowPath, $fedoraJobText, "2fa9c05d591a1582a9aba276272478c262e95ad00acf60eaee1644d93941e3c6", "Fedora verifier must bind the audited Microsoft signing-key payload hash"),
        @($workflowPath, $fedoraJobText, "BC528686B50D79E339D3721CEB3E94ADBE1229CF", "Fedora verifier must bind the audited full Microsoft signing-key fingerprint"),
        @($workflowPath, $fedoraJobText, "MICROSOFT_RPM_TRUST_EVIDENCE rid=fedora.40-x64", "Fedora verifier must emit verified local RPM trust evidence"),
        @($workflowPath, $fedoraJobText, "POWERSHELL_RPM_PACKAGE_NEVRA: powershell-7.6.4-1.rh.x86_64", "Fedora verifier must bind the audited exact PowerShell RPM NEVRA"),
        @($workflowPath, $fedoraJobText, "POWERSHELL_RPM_TOOLCHAIN_EVIDENCE rid=fedora.40-x64", "Fedora verifier must emit exact package-installed PowerShell evidence"),
        ...
        @($workflowPath, $fedoraJobText, '(archive\.fedoraproject\.org|fedora-archive|/archive).*/fedora/linux/releases/40/Everything/x86_64/os/', "Fedora verification must require an archive mirror serving the exact Fedora 40 release repository path"),
        @($workflowPath, $fedoraJobText, '(archive\.fedoraproject\.org|fedora-archive|/archive).*/fedora/linux/updates/40/Everything/x86_64/', "Fedora verification must require an archive mirror serving the exact Fedora 40 updates repository path"),
        ...
        @($workflowPath, $fedoraJobText, "dnf repoquery --disablerepo=`"*`" --enablerepo=`"packages-microsoft-com-prod`"", "Fedora verification must resolve the exact PowerShell RPM from the hardened Microsoft repo"),
        @($workflowPath, $fedoraJobText, "dnf install -y `"`$POWERSHELL_RPM_PACKAGE_NEVRA`"", "Fedora verification must install the exact audited PowerShell RPM before invoking repository guards"),
        @($workflowPath, $fedoraJobText, "test `"`$installed_powershell_package_identity`" = `"powershell|7.6.4-1.rh|x86_64|(none)`"", "Fedora verification must check the installed PowerShell RPM identity"),
        ...
        @($workflowPath, $fedoraJobText, "10.0.x", "Fedora verification must install a .NET 10 SDK from the 10.0 feature band"),
        @($workflowPath, $fedoraJobText, "9.0.316", "Fedora verification must install exact .NET 9 SDK 9.0.316"),
        @($workflowPath, $fedoraJobText, "8.0.423", "Fedora verification must install exact .NET 8 SDK 8.0.423"),
        ...
        @($workflowPath, $fedoraJobText, "expected_runtime_file_count=19", "Fedora mini package audit must require exactly 19 runtime files"),
        @($workflowPath, $fedoraJobText, "expected_canonical_count=7", "Fedora mini package audit must require exactly seven canonical ELFs"),
        ...
        @($workflowPath, $workflowText, 'FEDORA_40_X64_RUNTIME_INPUT_PROVENANCE_OK profile=mini files=$expectedPayloadFileCount modules=$expectedModuleCount sources=9 abi_functions=527 synthetic=false', "Pack runtime job must validate the exact Fedora 40 x64 mini producer provenance"),
        ...
        @($workflowPath, $rockyJobText, "container: rockylinux:9@sha256:d7be1c094cc5845ee815d4632fe377514ee6ebcf8efaed6892889657e5ddaaa6", "Rocky verification must execute in the audited Rocky Linux 9 job container"),
        @($workflowPath, $rockyJobText, "2fa9c05d591a1582a9aba276272478c262e95ad00acf60eaee1644d93941e3c6", "Rocky verifier must bind the audited Microsoft signing-key payload hash"),
        @($workflowPath, $rockyJobText, "BC528686B50D79E339D3721CEB3E94ADBE1229CF", "Rocky verifier must bind the audited full Microsoft signing-key fingerprint"),
        @($workflowPath, $rockyJobText, "MICROSOFT_RPM_TRUST_EVIDENCE rid=rocky.9-x64", "Rocky verifier must emit verified local RPM trust evidence"),
        @($workflowPath, $rockyJobText, "POWERSHELL_RPM_PACKAGE_NEVRA: powershell-7.6.4-1.rh.x86_64", "Rocky verifier must bind the audited exact PowerShell RPM NEVRA"),
        @($workflowPath, $rockyJobText, "POWERSHELL_RPM_TOOLCHAIN_EVIDENCE rid=rocky.9-x64", "Rocky verifier must emit exact package-installed PowerShell evidence"),
        ...
        @($workflowPath, $rockyJobText, "dnf repoquery --disablerepo=`"*`" --enablerepo=`"packages-microsoft-com-prod`"", "Rocky verification must resolve the exact PowerShell RPM from the hardened Microsoft repo"),
        @($workflowPath, $rockyJobText, "dnf install -y `"`$POWERSHELL_RPM_PACKAGE_NEVRA`"", "Rocky verification must install the exact audited PowerShell RPM before invoking repository guards"),
        @($workflowPath, $rockyJobText, "test `"`$installed_powershell_package_identity`" = `"powershell|7.6.4-1.rh|x86_64|(none)`"", "Rocky verification must check the installed PowerShell RPM identity"),
        ...
        @($workflowPath, $rockyJobText, "10.0.x", "Rocky verification must install a .NET 10 SDK from the 10.0 feature band"),
        @($workflowPath, $rockyJobText, "9.0.316", "Rocky verification must install exact .NET 9 SDK 9.0.316"),
        @($workflowPath, $rockyJobText, "8.0.423", "Rocky verification must install exact .NET 8 SDK 8.0.423"),
        ...
        @($workflowPath, $rockyJobText, "expected_runtime_file_count=19", "Rocky mini package audit must require exactly 19 runtime files"),
        @($workflowPath, $rockyJobText, "expected_canonical_count=7", "Rocky mini package audit must require exactly seven canonical ELFs"),
        ...
        @($workflowPath, $workflowText, 'ROCKY_9_X64_RUNTIME_INPUT_PROVENANCE_OK profile=mini files=$expectedPayloadFileCount modules=$expectedModuleCount sources=9 abi_functions=527 synthetic=false extra_cmake_args=empty', "Pack runtime job must validate exact Rocky 9 x64 mini producer provenance"),
        ...
        @($workflowPath, $rhelJobText, "inputs.rid == 'rhel.9-x64' && (inputs.runtime_profile == 'full' || inputs.runtime_profile == 'mini') && inputs.validate_synthetic_runtime != 'true' && inputs.publish_github_packages != 'true'", "RHEL verification must use the exact full/mini non-synthetic non-publishing gate"),
        @($workflowPath, $rhelJobText, "runs-on: ubuntu-24.04", "RHEL UBI verification must use the supported hosted runner as Docker host"),
        @($workflowPath, $rhelJobText, "container: registry.access.redhat.com/ubi9/ubi:9.8@sha256:50701171b9917ed51048b614924598d45b00bce9a64b73860c057922fc13bec2", "RHEL verification must execute in the audited official UBI 9.8 job container"),
        @($workflowPath, $rhelJobText, "2fa9c05d591a1582a9aba276272478c262e95ad00acf60eaee1644d93941e3c6", "RHEL verifier must bind the audited Microsoft signing-key payload hash"),
        @($workflowPath, $rhelJobText, "BC528686B50D79E339D3721CEB3E94ADBE1229CF", "RHEL verifier must bind the audited full Microsoft signing-key fingerprint"),
        @($workflowPath, $rhelJobText, "MICROSOFT_RPM_TRUST_EVIDENCE rid=rhel.9-x64", "RHEL verifier must emit verified local RPM trust evidence"),
        @($workflowPath, $rhelJobText, "POWERSHELL_RPM_PACKAGE_NEVRA: powershell-7.6.4-1.rh.x86_64", "RHEL verifier must bind the audited exact PowerShell RPM NEVRA"),
        @($workflowPath, $rhelJobText, "POWERSHELL_RPM_TOOLCHAIN_EVIDENCE rid=rhel.9-x64", "RHEL verifier must emit exact package-installed PowerShell evidence"),
        ...
        @($workflowPath, $rhelJobText, "RHEL_9_UBI_ASSEMBLER_EVIDENCE", "RHEL verification must retain assembler evidence for the separate full-profile producer workaround"),
        ...
        @($workflowPath, $rhelJobText, "dnf repoquery --disablerepo=`"*`" --enablerepo=`"packages-microsoft-com-prod`"", "RHEL verification must resolve the exact PowerShell RPM from the hardened Microsoft repo"),
        @($workflowPath, $rhelJobText, "dnf install -y `"`$POWERSHELL_RPM_PACKAGE_NEVRA`"", "RHEL verification must install the exact audited PowerShell RPM before invoking repository guards"),
        @($workflowPath, $rhelJobText, "test `"`$installed_powershell_package_identity`" = `"powershell|7.6.4-1.rh|x86_64|(none)`"", "RHEL verification must check the installed PowerShell RPM identity"),
        @($workflowPath, $rhelJobText, "10.0.x", "RHEL verification must install a .NET 10 SDK from the 10.0 feature band"),
        @($workflowPath, $rhelJobText, "9.0.316", "RHEL verification must install exact .NET 9 SDK 9.0.316"),
        @($workflowPath, $rhelJobText, "8.0.423", "RHEL verification must install exact .NET 8 SDK 8.0.423"),
        @($workflowPath, $rhelJobText, "name: nupkg-managed", "RHEL verification must download the same-run managed artifact explicitly"),
        @($workflowPath, $rhelJobText, 'name: nupkg-rhel.9-x64-${{ inputs.runtime_profile }}', "RHEL verification must download only the exact selected RHEL full/mini runtime artifact"),
        ...
        @($workflowPath, $rhelJobText, 'path: artifacts/pack-targeted-rhel/nupkg-rhel.9-x64-${{ inputs.runtime_profile }}', "RHEL runtime artifact must use its isolated exact profile path"),
        ...
        @($workflowPath, $rhelJobText, '-SelectedRuntimeProfile ''${{ inputs.runtime_profile }}''', "RHEL checks must select the exact full/mini profile"),
        @($workflowPath, $rhelJobText, "expected_runtime_file_count=19", "RHEL mini package audit must require exactly 19 runtime files"),
        @($workflowPath, $rhelJobText, "expected_canonical_count=7", "RHEL mini package audit must require exactly seven canonical ELFs"),
        @($workflowPath, $rhelJobText, "expected_direct_opencv=6", "RHEL mini package audit must require exactly six direct OpenCV dependencies"),
        @($workflowPath, $rhelJobText, "RHEL_9_X64_PACKAGE_ELF_EVIDENCE", "RHEL package must pass a profile-derived x86-64 ELF closure audit"),
        @($workflowPath, $workflowText, 'RHEL_9_X64_RUNTIME_INPUT_PROVENANCE_OK profile=mini files=$expectedPayloadFileCount modules=$expectedModuleCount sources=9 abi_functions=527 synthetic=false extra_cmake_args=empty', "Pack runtime job must validate exact RHEL 9 x64 mini producer provenance"),
        ...
        @($workflowPath, $alpineJobText, "POWERSHELL_ALPINE_PACKAGE_VERSION: 7.4.6-r1", "Alpine verifier must bind the audited exact PowerShell APK package version"),
        @($workflowPath, $alpineJobText, "POWERSHELL_ALPINE_SEMANTIC_VERSION: 7.4.6", "Alpine verifier must bind the audited PowerShell semantic version"),
        @($workflowPath, $alpineJobText, "powershell=`$POWERSHELL_ALPINE_PACKAGE_VERSION", "Alpine verification must install the exact official Alpine PowerShell package"),
        @($workflowPath, $alpineJobText, "POWERSHELL_APK_TOOLCHAIN_EVIDENCE rid=alpine.3.20-x64", "Alpine verifier must emit exact package-installed PowerShell evidence"),
...
Assert-OccurrenceCount -Path $workflowPath -Text $windowsX86JobText -Needle "inputs.rid ==" -ExpectedCount 1 -Issue "Windows x86 verifier must gate on exactly one RID"
Assert-OccurrenceCount -Path $workflowPath -Text $windowsX86JobText -Needle "inputs.runtime_profile ==" -ExpectedCount 1 -Issue "Windows x86 verifier must gate on only the full profile"
Assert-OccurrenceCount -Path $workflowPath -Text $windowsX86JobText -Needle "-SelectedRid win-x86" -ExpectedCount 2 -Issue "Both Windows x86 package guards must select exact win-x86"
Assert-OccurrenceCount -Path $workflowPath -Text $windowsX86JobText -Needle "-SelectedRuntimeProfile full" -ExpectedCount 2 -Issue "Both Windows x86 package guards must select only full"

foreach ($forbiddenWindowsX86Text in @(
        "win-x64",
        "win-arm64",
        ...
        "SetDllDirectory",
        '$env:PATH =')) {
    Assert-NotContains -Path $workflowPath -Text $windowsX86JobText -Needle $forbiddenWindowsX86Text -Issue "Windows x86 package verification must remain same-run, full-only, WoW64, and free of DLL search overrides: $forbiddenWindowsX86Text"
}
...
Assert-ExactLine `
    -Path $workflowPath `
    -Text $rhelJobText `
    -ExpectedLine "    if: `${{ inputs.rid == 'rhel.9-x64' && (inputs.runtime_profile == 'full' || inputs.runtime_profile == 'mini') && inputs.validate_synthetic_runtime != 'true' && inputs.publish_github_packages != 'true' }}" `
    -Issue "RHEL targeted verification condition must remain exactly RHEL 9 x64 full/mini"
...
Assert-OccurrenceCount `
    ...
    -Needle "inputs.runtime_profile ==" `
    -ExpectedCount 2 `
    -Issue "RHEL UBI container job must gate on exactly full and mini"
...
Assert-OccurrenceCount `
    ...
    -Needle "-SelectedRuntimeProfile '`${{ inputs.runtime_profile }}'" `
    -ExpectedCount 2 `
    -Issue "Both RHEL guards must select the exact full/mini profile"
...
Assert-Contains `
    ...
    -Text $rhelJobText `
    -Needle "runtime_profile == 'mini'" `
    -Issue "RHEL mini must enter the exact UBI container-native verification job"
