...
foreach ($required in @(
        ...
        [pscustomobject]@{ Needle = 'PRODUCER_ASSEMBLER_VERSION: ${{ steps.generic_linux_preview_runner.outputs.assembler_version || steps.ubuntu2204_x64_runner.outputs.assembler_version || steps.arm64_runner.outputs.assembler_version }}'; Issue = "Hosted Ubuntu producers must transport factual assembler evidence without interpolating tool output into PowerShell source" },
...
Assert-Contains -Violations $violations -Path $packWorkflowPath -Text $packWorkflowText -Needle "`$allowedRids = @('all') + @(`$matrix.rids | ForEach-Object { [string]`$_.rid })" -Issue "Pack workflow must reject unknown RID inputs before matrix jobs can report false success"
Assert-Contains -Violations $violations -Path $packWorkflowPath -Text $packWorkflowText -Needle "`$allowedProfiles = @('all') + @(`$matrix.profiles | ForEach-Object { [string]`$_.name })" -Issue "Pack workflow must reject unknown runtime profile inputs before matrix jobs can report false success"
Assert-Contains -Violations $violations -Path $packWorkflowPath -Text $packWorkflowText -Needle "win-x86/mini has no approved real runtime producer or target-execution evidence" -Issue "Pack workflow must reject the excluded win-x86/mini real candidate surface"
...
Assert-Contains -Violations $violations -Path $packWorkflowPath -Text $packWorkflowText -Needle "NativeAbiFunctionCount -ne 527" -Issue "Windows pack validation must require the exact mini ABI count"
