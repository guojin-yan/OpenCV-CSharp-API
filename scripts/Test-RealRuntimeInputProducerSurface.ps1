...
foreach ($required in @(
        ...
        [pscustomobject]@{ Needle = "expectedAbiFunctionCount = if (`$profileName -eq 'mini') { 526 } else { 2575 }"; Issue = "Windows producer must lock exact mini/full ABI counts" },
...
if ($violations.Count -eq 0) {
    ...
    try {
        ...
        foreach ($producerTarget in @(Get-RuntimeInputProducerTargets -Text $producerWorkflowText)) {
            ...
            $nativeAbiFunctionCount = if ($hasMiniProfileEvidence) { "526" } elseif ($hasFullProfileEvidence) { "2575" } else { "" }
