...
foreach ($required in @(
        ...
        [pscustomobject]@{ Needle = "expectedAbiFunctionCount = if (`$profileName -eq 'mini') { 526 } else { 2545 }"; Issue = "Windows producer must lock exact mini/full ABI counts" },
...
foreach ($windowsX86Needle in @(
        ...
        "_LINKED_CTEST_EVIDENCE passed=5 total=5",
...
if ($violations.Count -eq 0) {
    ...
    try {
        ...
        foreach ($producerTarget in @(Get-RuntimeInputProducerTargets -Text $producerWorkflowText)) {
            ...
            $nativeAbiFunctionCount = if ($hasMiniProfileEvidence) { "526" } elseif ($hasFullProfileEvidence) { "2545" } else { "" }
