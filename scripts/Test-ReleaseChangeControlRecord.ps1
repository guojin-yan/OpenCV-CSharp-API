...
function Test-ReviewRecord {
    ...
    Assert-True -List $List -Condition ($recordCloseoutValues -ceq $expectedCloseoutValues -and $Record.Closeout.Status -eq 'locally-validated' -and [int]$Record.Closeout.InvariantGuardCount -eq 80 -and $Record.Closeout.SigningStatus -eq 'repository-signing-pending' -and $Record.Closeout.ApprovalStatus -eq 'not-approved' -and -not [bool]$Record.Closeout.PublicationAllowed -and $Record.Closeout.Sha256 -match '^[0-9a-f]{64}$') -Path $Path -Issue 'Release review closeout binding drifted'
