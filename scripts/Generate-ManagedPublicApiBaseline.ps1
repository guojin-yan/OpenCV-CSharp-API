...
if ([string]::IsNullOrWhiteSpace($DotNetPath)) {
    ...
    if ($null -eq $dotnetCommand) {
        throw "dotnet was not found. Managed API baseline generation requires a .NET 10 SDK."
    }
...
if ($LASTEXITCODE -ne 0 -or $sdkVersion -notmatch '^10\.') {
    throw "Managed API baseline generation requires a .NET 10 SDK; resolved '$sdkVersion' from $resolvedDotNet."
}
