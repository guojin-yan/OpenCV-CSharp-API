...
if ([string]::IsNullOrWhiteSpace($DotNetPath)) {
    ...
    if ($null -eq $dotnetCommand) {
        throw "dotnet was not found. VideoIO upstream-map generation requires SDK 10.0.302."
    }
...
if ($LASTEXITCODE -ne 0 -or $sdkVersion -ne "10.0.302") {
    throw "VideoIO upstream-map generation requires exact SDK 10.0.302; resolved '$sdkVersion' from $resolvedDotNet."
}

$rawPath = Join-Path $repo "compatibility/videoio-upstream-raw.json"
$classificationPath = Join-Path $repo "compatibility/videoio-upstream-classifications.json"
$mappingPath = Join-Path $repo "compatibility/videoio-upstream-map.txt"
$summaryPath = Join-Path $repo "compatibility/videoio-upstream-summary.json"
$familyOutputPath = Join-Path $repo "compatibility/videoio-implemented-families.json"
...
if ($RegenerateRaw) {
    ...
    & $resolvedPython (Join-Path $repo "tools/VideoIOUpstreamMap/extract_videoio.py") `
        --workspace $workspace `
        --opencv-root $opencvRoot `
        --output $rawPath
    if ($LASTEXITCODE -ne 0) {
        throw "OpenCV hdr_parser.py VideoIO extraction failed with exit code $LASTEXITCODE."
    }
}

if (-not (Test-Path -LiteralPath $rawPath -PathType Leaf)) {
    throw "Checked VideoIO raw extraction was not found: $rawPath. Use -RegenerateRaw with an explicit working -PythonPath."
}

$arguments = @(
    "run",
    "--project", (Join-Path $repo "tools/VideoIOUpstreamMap/VideoIOUpstreamMap.csproj"),
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "VideoIO upstream-map generator failed with exit code $LASTEXITCODE."
    }
}
finally {
