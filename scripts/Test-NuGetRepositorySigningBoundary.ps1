...
try {
    ...
    $manifestRecord = [ordered]@{ SchemaVersion = 1; SourceRevision = ('a' * 40); PackageVersion = '5.0.0'; Packages = @($manifestPackages) }
    ...
    $manifestArguments = @('-ManifestPath', $manifestFixture, '-SourceCommit', ('a' * 40), '-PackageVersion', '5.0.0', '-OutputPath', $normalizedManifest)
    ...
    & pwsh -NoProfile -File $publicationManifest -ManifestPath $normalizedManifest -SourceCommit ('a' * 40) -PackageVersion '5.0.0' -OutputPath $normalizedManifest -Check
    Assert-True -Condition ($LASTEXITCODE -eq 0) -Path $publicationManifest -Issue 'Normalized publication manifest check failed'

    foreach ($invalidVersion in @('5.0.0-preview.1', '5.0.1', '5.0.0-preview.02')) {
    ...
    Invoke-PowerShellExpectedFailure -Name $uppercaseHashPath -Script $publicationManifest -Arguments @('-ManifestPath', $uppercaseHashPath, '-SourceCommit', ('a' * 40), '-PackageVersion', '5.0.0') -ExpectedText 'lowercase package SHA256'
...
"@, [Text.UTF8Encoding]::new($false))
    ...
    if (Test-Path -LiteralPath $workflow -PathType Leaf) {
        ...
        foreach ($token in @(
                ...
                "single-maintainer-stable-5.0.0-exception",
                "explicit-owner-authorized-stable-5.0.0-no-independent-reviewer",
                "The single-maintainer exception is restricted to the exact stable 5.0.0 release.",
