...
The managed surface currently has 611 public types and 6,300 public/protected members across 41 namespaces rooted at `JYPPX.OpenCvSharp`. The full native profile exposes 2,656 stable C ABI functions, and every current native function is bound by managed interop. Those numbers describe the current release baseline, not a claim that every OpenCV module is complete.

当前托管层包含 611 个公共类型、6,300 个 public/protected 成员和 `JYPPX.OpenCvSharp` 根下的 41 个命名空间。full native profile 暴露 2,656 个稳定 C ABI 函数，当前每个 native 函数均已有 managed interop 绑定。这些数字描述首版基线，不代表所有 OpenCV 模块都已完整封装。
