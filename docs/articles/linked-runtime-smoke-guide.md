...
For an MSBuild invocation, pass the directory explicitly as `/p:OpenCvNativeRuntimeDir=<runtime-native-dir>`.
...
These entries combine the single version-neutral loader with factual OpenCV runtime artifacts for the current packaged runtime identity, OpenCV 5.0.0.

这些条目包含唯一的版本中立 loader，以及当前打包 runtime 身份 OpenCV 5.0.0 的事实性 OpenCV runtime 产物。

- `JYPPX.OpenCV.Native.dll` (single loader)
