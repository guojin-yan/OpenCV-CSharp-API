...
The preview chain is an explicit manual dispatch of `.github/workflows/pack.yml`. Set `generic_linux_preview=true`, provide the successful `runtime-input.yml` run ID in `generic_linux_preview_artifact_run_id`, and choose `all`, `full`, or `mini` in `generic_linux_preview_profile`. The producer downloads only `runtime-input-ubuntu.22.04-x64-<profile>`, creates `.preview` runtime identities, and uploads isolated managed/runtime artifacts. Ubuntu 22.04, Debian 12, and Fedora 40 consumers then restore those artifacts and run native smoke independently. The dispatch path has no package-feed or release permission and cannot promote the result automatically.
...
RT-005 通过 `.github/workflows/pack.yml` 的手动 dispatch 执行：`generic_linux_preview=true`，填入成功的 `runtime-input.yml` run ID 到 `generic_linux_preview_artifact_run_id`，并选择 `all`、`full` 或 `mini`。producer 只下载 Ubuntu 22.04 x64 的真实 runtime-input，生成带 `.preview` 的隔离包；Ubuntu 22.04、Debian 12、Fedora 40 随后分别还原并执行 native smoke。该路径没有包 feed 或 Release 写权限，也不能自动 promotion。只有三发行版的 Full/Mini hosted producer、确定性包 hash、ELF/依赖闭包和 native smoke 证据全部完成后，RT-005 才能关闭；否则仍保持 preview-only，不修改 active 发行版矩阵。
