...
RT-006 uses the same explicit boundary for `linux-arm64`: [`runtime-generic-linux-arm64-preview-matrix.json`](../../packaging/runtime/runtime-generic-linux-arm64-preview-matrix.json) requires a native AArch64 `ubuntu-24.04-arm` host, an Ubuntu 22.04 ARM64 glibc producer container, and independent Ubuntu 22.04, Ubuntu 24.04, and Debian 12 ARM64 consumers. The package identities are `JYPPX.OpenCV.runtime.linux-arm64.preview` and `.preview.mini`; they are candidate artifacts only and never enter the active matrix or publication path.
