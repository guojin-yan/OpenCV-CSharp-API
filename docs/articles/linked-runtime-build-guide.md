...
`runtime-input.yml` is the first real producer workflow for this handoff. It produces full and proven mini `runtime-input-win-x64-*` on `windows-latest`, full `runtime-input-win-arm64-full` on `windows-11-vs2026-arm`, `runtime-input-ubuntu.24.04-x64-full` and `runtime-input-ubuntu.24.04-x64-mini` on `ubuntu-24.04`, plus the full-only real targets `runtime-input-ubuntu.22.04-x64-full`, `runtime-input-ubuntu.22.04-arm64-full`, `runtime-input-debian.12-x64-full`, `runtime-input-debian.12-arm64-full`, `runtime-input-fedora.40-x64-full`, `runtime-input-rhel.9-x64-full`, `runtime-input-rocky.9-x64-full`, and `runtime-input-alpine.3.20-x64-full`. The Ubuntu 22.04 and Debian 12 ARM64 targets build inside digest-pinned official ARM64 images on native `ubuntu-24.04-arm` hardware. The RHEL target builds inside the official `registry.access.redhat.com/ubi9/ubi:9.8` image and requires factual UBI BaseOS, AppStream, and CodeReady Builder repositories. The Alpine target builds only inside `alpine:3.20`, uses its main/community repositories, `samurai`, `linux-headers`, and factual musl evidence. Each target fetches factual OpenCV source, builds the selected profile build list, links/tests the matching profile of `JYPPX.OpenCV.Native`, records platform/profile/build-list provenance, and uploads `runtime-input-<rid>-<profile>` with `native-wrapper/`, `opencv-runtime/`, `opencv-source/`, and optional `opencv-install/`. The containerized distro producers record hosted-runner, container image, actual container distro/version, and libc evidence. Remaining mini producers, including Windows x86/ARM64, Ubuntu and Debian ARM64, and Alpine, stay disabled until their own boundaries are verified. After a successful producer run, pass its run id to `pack.yml` through `real_runtime_artifact_run_id` with `validate_synthetic_runtime=false` and `publish_github_packages=false`.
...
该真实执行 allowlist 还包含单独的 `verify-targeted-real-windows-arm64`：它在 `windows-11-vs2026-arm` 的原生 ARM64 进程中只下载同 run 的 managed 与 `win-arm64/full` artifacts，执行精确 package/PE guard、隔离 restore/build 和 full DNN smoke，并拒绝 producer `PATH`、`AddDllDirectory` 与 runtime-root override。
