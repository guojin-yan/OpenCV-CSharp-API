...
`runtime-input.yml` is the first real producer workflow for this handoff. It currently produces `runtime-input-ubuntu.24.04-x64-full` on `ubuntu-24.04`, `runtime-input-ubuntu.22.04-x64-full` on `ubuntu-22.04`, and `runtime-input-debian.12-x64-full` by building inside a `debian:12` container on an Ubuntu hosted runner. Each target fetches factual OpenCV source, builds OpenCV with the full profile build list, links/tests `JYPPX.OpenCV.Native`, records distro/profile/build-list provenance, and uploads `runtime-input-<rid>-<profile>` with `native-wrapper/`, `opencv-runtime/`, `opencv-source/`, and optional `opencv-install/`. The Debian producer records hosted-runner, container image, actual container distro/version, and libc evidence so it is not treated as a generic Ubuntu-hosted runtime. Mini and other distro-specific RID producers remain intentionally disabled until their linked-component or distro-native boundaries are implemented and verified. After a successful producer run, pass its run id to `pack.yml` through `real_runtime_artifact_run_id` with `validate_synthetic_runtime=false` and `publish_github_packages=false`.
...
`runtime-input.yml` 是这条 handoff 的第一条真实 producer workflow。当前它会在 `ubuntu-24.04` 上生产 `runtime-input-ubuntu.24.04-x64-full`，在 `ubuntu-22.04` 上生产 `runtime-input-ubuntu.22.04-x64-full`，并在 Ubuntu hosted runner 中通过 `debian:12` 容器生产 `runtime-input-debian.12-x64-full`。每个目标都会获取事实性 OpenCV 源码，按 full profile build list 构建 OpenCV，链接/测试 `JYPPX.OpenCV.Native`，记录 distro/profile/build-list provenance，并上传带有 `native-wrapper/`、`opencv-runtime/`、`opencv-source/` 与可选 `opencv-install/` 的 `runtime-input-<rid>-<profile>`。Debian producer 会记录 hosted runner、container image、实际 container distro/version 和 libc 证据，避免把它当成通用 Ubuntu-hosted runtime。mini 和其他 distro-specific RID producer 会保持明确禁用，直到对应 linked-component 或 distro-native 边界完成并通过验证。producer run 成功后，把它的 run id 通过 `real_runtime_artifact_run_id` 传给 `pack.yml`，并使用 `validate_synthetic_runtime=false` 与 `publish_github_packages=false`。
