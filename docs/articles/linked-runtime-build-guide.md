...
The full-only `runtime-input-debian.12-arm64-full` producer also keeps actions on native AArch64 `ubuntu-24.04-arm` and builds only inside official Debian 12 ARM64 userspace pinned to the audited multi-architecture digest. Provenance records the host runner/image/distro/architecture/libc/CPU/disk boundary, resolved image ID/digest, Debian 12, `aarch64`/`arm64`, glibc 2.36, the verified PowerShell 7.4.17 ARM64 archive SHA256, empty extra CMake arguments, and factual OpenCV NEON configuration. Linked CTest must pass 5/5, and two loaders plus 16 canonical OpenCV ELFs must pass AArch64, `$ORIGIN`, zero producer path, 16 direct dependency, and zero missing dependency checks. x64, QEMU, `--platform`, emulation, cross-builds, and Debian ARM64 mini are excluded.
...
Debian 12 ARM64 full 使用由原生 `ubuntu-24.04-arm` 宿主机编排的独立 `docker run` verifier。actions 与精确的同 run 下载保留在宿主机；digest 固定的官方 Debian 12 ARM64 容器先核验 Debian 12、`aarch64`/`arm64`、glibc、PowerShell archive hash 与原生 .NET 8，再运行精确 artifact/provenance guard、隔离 restore/build、包内 loader 和确定性 DNN，且不设置 `LD_LIBRARY_PATH`。x64、QEMU、`--platform`、模拟、交叉构建和 ARM64 mini 均不能作为证据。
...
`runtime-input.yml` 是这条 handoff 的第一条真实 producer workflow。当前它会在 `windows-latest` 上生产 full 与已经验证的 mini `runtime-input-win-x64-*`，在 `windows-11-vs2026-arm` 上生产 full 与已经验证的 mini `runtime-input-win-arm64-*`，在 `ubuntu-24.04` 上生产 full 与 mini `runtime-input-ubuntu.24.04-x64-*` target；`runtime-input-ubuntu.22.04-arm64-full` 与 `runtime-input-ubuntu.22.04-arm64-mini` 在原生 `ubuntu-24.04-arm` 宿主机上的 digest 固定官方 Ubuntu 22.04 ARM64 容器中构建。Ubuntu 22.04 x64、Debian 12 x64/ARM64、Fedora 40、RHEL 9、Rocky 9 和 Alpine 3.20 当前仍是 full-only 真实 producer，其中 `runtime-input-debian.12-arm64-full` 也只在原生 AArch64 宿主机上的 digest 固定官方 ARM64 容器中构建。RHEL 目标在官方 `registry.access.redhat.com/ubi9/ubi:9.8` 中构建，并要求事实 UBI BaseOS、AppStream 与 CodeReady Builder 仓库；Alpine 目标只在 `alpine:3.20` 中构建，要求 v3.20 main/community、`samurai`、`linux-headers` 与 musl 证据。每个目标都会获取事实性 OpenCV 源码，按所选 profile build list 构建 OpenCV，链接/测试匹配 profile 的 `JYPPX.OpenCV.Native`，记录 platform/profile/build-list provenance，并上传带有 `native-wrapper/`、`opencv-runtime/`、`opencv-source/` 与可选 `opencv-install/` 的 `runtime-input-<rid>-<profile>`。容器化 distro producer 会记录 hosted runner、container image、实际 container distro/version 和 libc 证据。Windows x86、Debian ARM64 以及 Alpine 的其余 mini producer 会保持禁用，直到各自边界完成并通过验证。producer run 成功后，把它的 run id 通过 `real_runtime_artifact_run_id` 传给 `pack.yml`，并使用 `validate_synthetic_runtime=false` 与 `publish_github_packages=false`。
...
full-only 的 `runtime-input-debian.12-arm64-full` producer 同样把 actions 保留在原生 AArch64 `ubuntu-24.04-arm` 宿主机，只在 digest 固定的官方 Debian 12 ARM64 userspace 内构建。provenance 记录宿主 runner/image/distro/architecture/libc/CPU/disk、解析后的 image ID/digest、Debian 12、`aarch64`/`arm64`、glibc 2.36、经 SHA256 核验的 PowerShell 7.4.17 ARM64 archive、空 extra CMake arguments 和事实 OpenCV NEON 配置；linked CTest 必须 5/5，通过两个 loader 加 16 个 canonical OpenCV ELF 的 AArch64、`$ORIGIN`、零 producer path、16 条直接依赖和零缺失依赖审计。x64、QEMU、`--platform`、模拟、交叉构建和 Debian ARM64 mini 均被排除。
