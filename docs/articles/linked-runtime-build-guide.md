...
`runtime-input.yml` 是这条 handoff 的第一条真实 producer workflow。当前它会在 `ubuntu-24.04` 上生产 `runtime-input-ubuntu.24.04-x64-full`，并在 `ubuntu-22.04` 上生产 `runtime-input-ubuntu.22.04-x64-full`：每个目标都会获取事实性 OpenCV 源码，按 full profile build list 构建 OpenCV，链接/测试 `JYPPX.OpenCV.Native`，记录 distro/profile/build-list provenance，并上传带有 `native-wrapper/`、`opencv-runtime/`、`opencv-source/` 与可选 `opencv-install/` 的 `runtime-input-<rid>-<profile>`。mini 和非 Ubuntu distro-specific RID producer 会保持明确禁用，直到对应 linked-component 或 distro-native 边界完成并通过验证。producer run 成功后，把它的 run id 通过 `real_runtime_artifact_run_id` 传给 `pack.yml`，并使用 `validate_synthetic_runtime=false` 与 `publish_github_packages=false`。
