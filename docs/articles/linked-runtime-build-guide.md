...
pack workflow 暴露 `rid` 和 `runtime_profile` 输入。`all` 会运行配置好的 full/mini 多 RID 矩阵；也支持定向 RID/profile 打包。workflow 从 `artifacts/packages` 上传中性的 `nupkg-*` 产物，先用 `Test-GitHubPackArtifactMatrixSurface.ps1` 自检 full matrix artifacts，再用 `Test-GitHubPackConsumerRestoreSurface.ps1` 验证 consumer restore/build。真实 non-synthetic `ubuntu.24.04-x64/mini` 定向 run 还会只下载同一 run 的 managed/runtime artifacts，校验精确的 20 文件、六模块 SONAME payload 与 provenance，并通过包内 loader 实际执行 core/imgproc/imgcodecs/videoio 调用；Linux loader 使用 `$ORIGIN` RUNPATH，不保留 producer build path。默认 synthetic runtime inputs 只验证 package shape，并在启用时拒绝发布。
