...
`runtime-input.yml` 是这条 handoff 的第一条真实 producer workflow。当前它会在 `windows-latest` 上生产 full 与已经验证的 mini `runtime-input-win-x64-*`，在 `windows-11-vs2026-arm` 上生产 full 与已经验证的 mini `runtime-input-win-arm64-*`，在 `ubuntu-24.04` 上生产 full 与 mini `runtime-input-ubuntu.24.04-x64-*` target；`runtime-input-ubuntu.22.04-arm64-full` 与 `runtime-input-ubuntu.22.04-arm64-mini` 在原生 `ubuntu-24.04-arm` 宿主机上的 digest 固定官方 Ubuntu 22.04 ARM64 容器中构建；`runtime-input-debian.12-arm64-full` 与 `runtime-input-debian.12-arm64-mini` 在同一原生宿主机上的 digest 固定官方 Debian 12 ARM64 容器中构建。Ubuntu 22.04 x64、Debian 12 x64、Fedora 40、RHEL 9、Rocky 9 和 Alpine 3.20 当前仍是 full-only 真实 producer。RHEL 目标在官方 `registry.access.redhat.com/ubi9/ubi:9.8` 中构建，并要求事实 UBI BaseOS、AppStream 与 CodeReady Builder 仓库；Alpine 目标只在 `alpine:3.20` 中构建，要求 v3.20 main/community、`samurai`、`linux-headers` 与 musl 证据。每个目标都会获取事实性 OpenCV 源码，按所选 profile build list 构建 OpenCV，链接/测试匹配 profile 的 `JYPPX.OpenCV.Native`，记录 platform/profile/build-list provenance，并上传带有 `native-wrapper/`、`opencv-runtime/`、`opencv-source/` 与可选 `opencv-install/` 的 `runtime-input-<rid>-<profile>`。容器化 distro producer 会记录 hosted runner、container image、实际 container distro/version 和 libc 证据。Windows x86 与 Alpine 的其余 mini producer 会保持禁用，直到各自边界完成并通过验证。producer run 成功后，把它的 run id 通过 `real_runtime_artifact_run_id` 传给 `pack.yml`，并使用 `validate_synthetic_runtime=false` 与 `publish_github_packages=false`。
