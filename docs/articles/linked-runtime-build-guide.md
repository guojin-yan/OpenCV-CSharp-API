...
`pack.yml` does not currently build or download real runtime inputs. When `validate_synthetic_runtime=false`, real input paths must already exist on the selected runner: `native_runtime_dir`, `opencv_runtime_dir`, and `opencv_source_dir` are required directories, while `opencv_install_dir` is optional but must exist when provided. Synthetic runtime inputs are package-surface validation only; real publishable runtime packages require `SyntheticRuntimeInputs=false` provenance and release preflight. The next real-GitHub handoff should add an explicit build/download artifact step before disabling synthetic validation.
...
`pack.yml` 当前不会构建或下载真实 runtime 输入。当 `validate_synthetic_runtime=false` 时，真实输入路径必须已经存在于 selected runner：`native_runtime_dir`、`opencv_runtime_dir` 和 `opencv_source_dir` 是必需目录，`opencv_install_dir` 是可选目录但如果提供也必须存在。synthetic runtime inputs 只用于 package-surface validation；真实可发布 runtime 包必须带有 `SyntheticRuntimeInputs=false` provenance 并通过 release preflight。下一步真实 GitHub handoff 应在关闭 synthetic validation 前增加明确的 build/download artifact 步骤。
