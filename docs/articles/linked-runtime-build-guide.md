...
`runtime-input.yml` is the first real producer workflow for this handoff. It currently produces only `runtime-input-ubuntu.24.04-x64-full`: the job fetches factual OpenCV source, builds OpenCV with the full profile build list, links/tests `JYPPX.OpenCV.Native`, and uploads `runtime-input-<rid>-<profile>` with `native-wrapper/`, `opencv-runtime/`, `opencv-source/`, and optional `opencv-install/`. Mini and additional distro-specific RID producers remain intentionally disabled until their linked-component boundaries are implemented and verified. After a successful producer run, pass its run id to `pack.yml` through `real_runtime_artifact_run_id` with `validate_synthetic_runtime=false` and `publish_github_packages=false`.
...
`runtime-input.yml` 是这条 handoff 的第一条真实 producer workflow。当前它只生产 `runtime-input-ubuntu.24.04-x64-full`：job 会获取事实性 OpenCV 源码，按 full profile build list 构建 OpenCV，链接/测试 `JYPPX.OpenCV.Native`，并上传带有 `native-wrapper/`、`opencv-runtime/`、`opencv-source/` 与可选 `opencv-install/` 的 `runtime-input-<rid>-<profile>`。mini 和其它 distro-specific RID producer 会保持明确禁用，直到对应 linked-component 边界完成并通过验证。producer run 成功后，把它的 run id 通过 `real_runtime_artifact_run_id` 传给 `pack.yml`，并使用 `validate_synthetic_runtime=false` 与 `publish_github_packages=false`。
