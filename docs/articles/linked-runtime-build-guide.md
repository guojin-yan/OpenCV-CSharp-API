...
The full-only `runtime-input-ubuntu.24.04-arm64-full` producer runs directly on `ubuntu-24.04-arm`. It records the actual runner image, Ubuntu 24.04, `aarch64`/`arm64`, glibc, CPU, disk, and OpenCV NEON configuration in runtime-input provenance; requires linked CTest 5/5; and audits two loaders plus 16 canonical OpenCV ELFs for AArch64, `$ORIGIN`, zero producer paths, 16 direct OpenCV dependencies, and zero missing dependencies. No x86 CPU workaround is applied, and Ubuntu ARM64 mini remains synthetic package-surface-only.
...
Ubuntu 24.04 ARM64 full runs natively on `ubuntu-24.04-arm` in a separate exact verifier. It requires Ubuntu 24.04, `aarch64`, Debian `arm64`, and glibc evidence, downloads only same-run managed and `ubuntu.24.04-arm64/full` artifacts, requires real provenance, and executes the full native/DNN smoke without `LD_LIBRARY_PATH`. x64 execution, cross-compilation, QEMU, containers, and ARM64 mini are excluded from this evidence boundary.
...
Ubuntu 24.04 ARM64 full 使用单独的 `ubuntu-24.04-arm` 原生 verifier。它要求 Ubuntu 24.04、`aarch64`、Debian `arm64` 与 glibc 证据，只下载同 run 的 managed 和 `ubuntu.24.04-arm64/full` artifacts，要求真实 provenance，并在不设置 `LD_LIBRARY_PATH` 的情况下执行 full native/DNN smoke。x64 执行、交叉编译、QEMU、容器和 ARM64 mini 都不属于这条证据边界。
...
full-only 的 `runtime-input-ubuntu.24.04-arm64-full` producer 直接运行在 `ubuntu-24.04-arm`。它会在 runtime-input provenance 中记录实际 runner image、Ubuntu 24.04、`aarch64`/`arm64`、glibc、CPU、磁盘和 OpenCV NEON 配置；要求 linked CTest 5/5；并对两个 loader 与 16 个 canonical OpenCV ELF 审计 AArch64、`$ORIGIN`、零 producer path、16 条 OpenCV 直接依赖和零缺失依赖。该 producer 不使用 x86 CPU workaround，Ubuntu ARM64 mini 仍只做 synthetic package-surface validation。
