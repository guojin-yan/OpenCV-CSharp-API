...
The preflight test suite includes a deterministic malformed corpus: every strict prefix of complete PNG, JPEG, GIF, APNG, WebP, BMP, classic TIFF, and BigTIFF fixtures is inspected, together with oversized container lengths and broken TIFF directory pointers. These cases must return unknown affected facts without leaking indexing, arithmetic, or native exceptions. This is evidence for the managed parser's bounded behavior only; native decoder isolation, timeout, and peak-allocation controls remain separate deployment responsibilities.

预检测试套件包含确定性的 malformed corpus：对完整 PNG、JPEG、GIF、APNG、WebP、BMP、经典 TIFF 和 BigTIFF fixture 的每一个严格前缀执行识别，并加入超大容器长度和损坏 TIFF 目录指针。此类输入必须将受影响事实置为未知，不能泄漏索引、算术或 native 异常。这只证明 managed 解析器的有界行为；native 解码隔离、超时和峰值分配控制仍属于部署侧职责。
