...
On Span-capable target frameworks, the preview writer overload writes the native encoded result into an `IBufferWriter<byte>` and advances it once. It does not retain the writer or its memory:

```csharp
var destination = new System.Buffers.ArrayBufferWriter<byte>();
Cv2.ImEncodeTo(".png", image, destination);
ReadOnlySpan<byte> encoded = destination.WrittenSpan;
```

在支持 Span 的目标框架上，preview writer overload 将 native 编码结果写入 `IBufferWriter<byte>`，并只调用一次 `Advance`；不会保留 writer 或其内存。
...
The same 14 complete format fixtures also feed a deterministic byte-mutation corpus, recorded in [`image-preflight-mutation-corpus.json`](../../packaging/codec/image-preflight-mutation-corpus.json). Every byte is replaced in turn with each of `00`, `01`, `7F`, `80`, and `FF` (excluding no-op replacements), on both `net8.0` and `net10.0`. The test requires the parser to return a result without throwing and checks that every reported fact is internally positive, bounded, and supported by the prerequisite facts. A companion stream test applies first/middle/last-byte mutations to seekable and short-read non-seekable streams, requiring seek restoration or complete consumption. Run the focused corpus with `scripts/Test-ImagePreflightMutationCorpus.ps1`; each framework has a hard timeout. This is a reproducible managed parser robustness check, not a coverage-guided fuzzer or proof against every possible input.
...
同样的 14 种完整格式 fixture（清单见 [`image-preflight-mutation-corpus.json`](../../packaging/codec/image-preflight-mutation-corpus.json)）还用于确定性的 byte mutation corpus：逐字节替换为 `00`、`01`、`7F`、`80`、`FF`（跳过无变化替换），并在 `net8.0` 和 `net10.0` 上运行。测试要求解析器不抛异常并返回结果，同时验证每个已报告事实均为正值、有界且具备相应前置事实。配套 stream 测试对每种格式的首/中/尾字节执行 mutation，覆盖 seekable 与短读 non-seekable 流，要求恢复位置或完整消费。使用 `scripts/Test-ImagePreflightMutationCorpus.ps1` 运行 focused corpus；每个框架都有硬超时。这是可重复的 managed parser 鲁棒性检查，不是 coverage-guided fuzzer，也不证明覆盖所有可能输入。
