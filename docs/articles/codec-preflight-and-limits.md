...
同样的 13 种完整格式 fixture 还用于确定性的 byte mutation corpus：逐字节替换为 `00`、`01`、`7F`、`80`、`FF`（跳过无变化替换），并在 `net8.0` 和 `net10.0` 上运行。测试要求解析器不抛异常并返回结果，同时验证每个已报告事实均为正值、有界且具备相应前置事实。使用 `scripts/Test-ImagePreflightMutationCorpus.ps1` 运行 focused corpus；每个框架都有硬超时。这是可重复的 managed parser 鲁棒性检查，不是 coverage-guided fuzzer，也不证明覆盖所有可能输入。
