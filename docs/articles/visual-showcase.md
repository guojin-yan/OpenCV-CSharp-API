...
For a post-release check, use the grouped [`samples`](https://github.com/guojin-yan/OpenCV-CSharp-API/tree/opencv5.x/samples) fixture. It restores the latest public stable managed and Windows x64 runtime packages and runs each case without a project reference. See [Example Catalog](example-catalog.md) for the complete command and evidence interpretation.

发布后验证请使用分组后的 [`samples`](https://github.com/guojin-yan/OpenCV-CSharp-API/tree/opencv5.x/samples) 夹具。它从公开源恢复最新稳定版 managed 包和 Windows x64 runtime 包，不使用项目引用即可逐个运行案例。完整命令和证据解释见[案例目录](example-catalog.md)。
