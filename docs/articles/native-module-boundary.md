...
| ml | `TrainData`, `ParamGrid`, `KNearest`, `SVM`, `NormalBayesClassifier`, `DTrees`, `RTrees`, `Boost`, `ANN_MLP` | Local OpenCV 5.0.0 exposes this through contrib; samples usually use `CV_32F` data and runtime needs the factual OpenCV 5.0.0 runtime artifact `opencv_ml500.dll` |
...
- ML mode: in the local OpenCV 5.0.0 tree, `opencv_ml` comes from contrib and is required for `TrainData`, `KNearest`, `SVM`, `NormalBayesClassifier`, `DTrees`, `RTrees`, `Boost`, and `ANN_MLP`.
