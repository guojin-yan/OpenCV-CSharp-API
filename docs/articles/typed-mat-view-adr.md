...
4. All 15 package target frameworks compile; the public view is conditionally available only on Span-capable targets. `scripts/Test-TypedMatTargetFrameworks.ps1` records Release builds from net46 through net10.0, while `tools/TypedMatBenchmark` builds on net8.0 and net10.0 and compares MatRowAccessor<T>, MatView<TPixel>, and ReadOnlyMatView<TPixel> over a fixed ROI. The local guard emits `skipped` when no factual native runtime is present; measured allocation/throughput evidence is still required before stabilization.
