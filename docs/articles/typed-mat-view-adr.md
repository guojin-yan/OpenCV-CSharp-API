...
4. All package target frameworks compile, but the public view is conditionally available only on Span-capable targets. `tools/TypedMatBenchmark` now builds on net8.0 and net10.0 and compares MatRowAccessor<T>, MatView<TPixel>, and ReadOnlyMatView<TPixel> over a fixed ROI. The local guard emits `skipped` when no factual native runtime is present; measured allocation/throughput evidence is still required before stabilization.
