...
- Run `scripts/Test-ReleaseReadinessContract.ps1` and `scripts/Test-ReleaseSigningBoundary.ps1`; they validate the versioned signing/SBOM state machine, normalized-input/post-signing package hash binding, immutable public references, explicit reviewer/approver scope, private-key exclusion, read-only feed policy, and the hosted-only `win-x86/full` promotion checklist.
...
- Run `scripts/Test-ReleaseChangeControlRecord.ps1`; it binds package hashes, source/runtime-matrix revisions, evidence guards, approval, signing/SBOM, rollback, and publication state in a deterministic review record, while keeping historical local artifacts non-publishable.
...
- 运行 `scripts/Test-ReleaseReadinessContract.ps1` 与 `scripts/Test-ReleaseSigningBoundary.ps1`；它们会校验版本化 signing/SBOM state machine、normalized-input/post-signing package hash 绑定、不可变 public reference、明确 reviewer/approver scope、private-key 排除、只读 feed policy，以及 hosted-only `win-x86/full` promotion checklist。
...
- 运行 `scripts/Test-ReleaseChangeControlRecord.ps1`；它会在 deterministic review record 中绑定 package hash、source/runtime-matrix revision、evidence guard、approval、signing/SBOM、rollback 与 publication state，同时保持历史本地 artifact 不可发布。
