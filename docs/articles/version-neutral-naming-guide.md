...
- Run `scripts/Test-RealRuntimeInputProducerSurface.ps1`; it verifies the first real `runtime-input.yml` producer builds OpenCV, links/tests `JYPPX.OpenCV.Native`, uploads the neutral `runtime-input-linux-x64-full` handoff layout, and does not use synthetic runtime inputs or package publishing.
...
- 运行 `scripts/Test-RealRuntimeInputProducerSurface.ps1`；它会校验第一条真实 `runtime-input.yml` producer 会构建 OpenCV，链接/测试 `JYPPX.OpenCV.Native`，上传中性的 `runtime-input-linux-x64-full` handoff layout，并且不使用 synthetic runtime inputs 或 package publishing。
