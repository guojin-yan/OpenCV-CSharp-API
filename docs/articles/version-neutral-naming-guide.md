...
- Run `scripts/Test-WorkflowInvariantCoverage.ps1`; it verifies managed, pack, docs, and native workflows run the aggregate invariant suite before restore, build, pack, DocFX, or CMake work begins, and keeps GitHub workflow/PR/issue/release surfaces free of fixed-major project identities.
...
- Run `scripts/Test-RuntimeRidPackageTemplateScalability.ps1`; it verifies `JYPPX.OpenCV.runtime.<rid>` package IDs, `RuntimePackageRid`-driven runtime payload paths, pack workflow RID inputs, and current concrete `win-x64` examples stay scalable to additional RID projects.
...
- Run `scripts/Test-RuntimePackageAvailabilityFallbackGuidance.ps1`; it verifies availability docs name the currently tracked runtime package project, do not describe future RID packages as published, and point users without a matching runtime package to `Build-OpenCV.ps1`, `Stage-Runtime.ps1`, `OpenCvNativeRuntimeDir`, and `-OpenCvNativeRuntimeDir`.
...
- 运行 `scripts/Test-WorkflowInvariantCoverage.ps1`；它会校验 managed、pack、docs 与 native workflow 在 restore、build、pack、DocFX 或 CMake 工作开始前运行总不变量套件，并确保 GitHub workflow/PR/issue/release 表面没有固定 major 项目身份。
...
- 运行 `scripts/Test-RuntimeRidPackageTemplateScalability.ps1`；它会校验 `JYPPX.OpenCV.runtime.<rid>` package ID、由 `RuntimePackageRid` 驱动的 runtime payload 路径、pack workflow RID 输入，以及当前具体 `win-x64` 示例是否仍可扩展到其他 RID 项目。
...
- 运行 `scripts/Test-RuntimePackageAvailabilityFallbackGuidance.ps1`；它会校验 availability docs 是否命名当前跟踪的 runtime package project、是否避免把未来 RID 包描述成已发布，以及是否把没有匹配 runtime 包的用户引导到 `Build-OpenCV.ps1`、`Stage-Runtime.ps1`、`OpenCvNativeRuntimeDir` 和 `-OpenCvNativeRuntimeDir`。
...
- 运行 `scripts/Test-RuntimePackageDocsDiscoverability.ps1`；它会校验 runtime package docs 是否通过 [Quick Start](quick-start.md)、[Linked Runtime Build Guide](linked-runtime-build-guide.md)、[Linked Runtime Smoke Guide](linked-runtime-smoke-guide.md)、[Smoke Profiles Guide](smoke-profiles-guide.md)、[Runtime Licenses](runtime-licenses.md) 和当前具体 [win-x64 runtime package README](../../packaging/runtime/JYPPX.OpenCV.runtime.win-x64/README.md) 保持交叉链接，同时不隐藏 `JYPPX.OpenCV.runtime.<rid>` 或 no-matching-runtime-package fallback。
