...
`runtime-input.yml` is the first real producer workflow for that handoff. It currently produces proven full and mini `runtime-input-win-x64-*` on `windows-latest`, proven full and mini `runtime-input-win-arm64-*` on `windows-11-vs2026-arm`, `runtime-input-ubuntu.24.04-x64-full` and `runtime-input-ubuntu.24.04-x64-mini` on `ubuntu-24.04`, `runtime-input-ubuntu.22.04-x64-full` on `ubuntu-22.04`, `runtime-input-ubuntu.22.04-arm64-full` inside official Ubuntu 22.04 ARM64 userspace on `ubuntu-24.04-arm`, `runtime-input-debian.12-x64-full` inside `debian:12`, `runtime-input-debian.12-arm64-full` inside official Debian 12 ARM64 userspace on `ubuntu-24.04-arm`, `runtime-input-fedora.40-x64-full` inside `fedora:40`, `runtime-input-rhel.9-x64-full` inside the official `registry.access.redhat.com/ubi9/ubi:9.8` image, `runtime-input-rocky.9-x64-full` inside `rockylinux:9`, and `runtime-input-alpine.3.20-x64-full` inside `alpine:3.20` on Ubuntu hosted Docker runners. Each target fetches factual OpenCV source, builds the selected OpenCV profile, links/tests the matching profile of `JYPPX.OpenCV.Native`, records platform/profile/build-list provenance, and uploads the agreed `runtime-input-<rid>-<profile>` layout. The containerized distro producers additionally record hosted-runner, container image, container distro/version, and libc evidence so they are not confused with generic Ubuntu-hosted package-surface runs. Mini producers for the remaining RIDs, including Windows x86, Ubuntu and Debian ARM64, and Alpine, stay disabled until their own linked-component and platform boundaries are verified.
...
`runtime-input.yml` 是该 handoff 的第一条真实 producer workflow。当前它会在 `windows-latest` 上生产已经验证的 full 与 mini `runtime-input-win-x64-*`，在 `windows-11-vs2026-arm` 上生产已经验证的 full 与 mini `runtime-input-win-arm64-*`，在 `ubuntu-24.04` 上生产 full 与 mini `runtime-input-ubuntu.24.04-x64-*` artifact，在 `ubuntu-22.04` 上生产 `runtime-input-ubuntu.22.04-x64-full`，在原生 `ubuntu-24.04-arm` 宿主机的官方 Ubuntu 22.04 ARM64 容器中生产 `runtime-input-ubuntu.22.04-arm64-full`，在同类原生宿主机的官方 Debian 12 ARM64 容器中生产 `runtime-input-debian.12-arm64-full`，并通过 Ubuntu hosted runner 中的 `debian:12`、`fedora:40`、官方 `registry.access.redhat.com/ubi9/ubi:9.8`、`rockylinux:9` 与 `alpine:3.20` 容器分别生产对应的 x64 full artifact。每个目标都会获取事实性 OpenCV 源码、构建所选 OpenCV profile、链接并测试匹配 profile 的 `JYPPX.OpenCV.Native`，记录 platform/profile/build-list provenance，然后上传约定的 `runtime-input-<rid>-<profile>` layout。容器化 distro producer 还会记录 hosted runner、container image、container distro/version 和 libc 证据。Windows x86、Ubuntu 与 Debian ARM64 以及 Alpine 的其余 mini producer 均保持禁用，直到各自 linked-component 与平台边界完成并通过验证。
...
Ubuntu 22.04 ARM64 full runs through a separate host-orchestrated `docker run` verifier on `ubuntu-24.04-arm`. Checkout and same-run artifact downloads remain on the native AArch64 host; exact artifact/provenance checks, isolated .NET 8 ARM64 restore/build, packaged loader execution, and deterministic DNN run inside the digest-pinned official Ubuntu 22.04 ARM64 userspace without `LD_LIBRARY_PATH`, QEMU, `--platform`, or cross-compilation.

Debian 12 ARM64 full uses its own host-orchestrated `docker run` verifier on native `ubuntu-24.04-arm`. Checkout and exact same-run artifact downloads remain on the AArch64 host; the digest-pinned official Debian 12 ARM64 container verifies Debian 12, `aarch64`/`arm64`, glibc, the PowerShell archive hash, and native .NET 8 before running exact artifact/provenance guards, isolated restore/build, packaged loader execution, and deterministic DNN without `LD_LIBRARY_PATH`. x64, QEMU, `--platform`, emulation, cross-builds, and ARM64 mini are excluded.
...
To try a real producer/consumer chain, dispatch `runtime-input.yml`; `win-x64`, `win-arm64`, and `ubuntu.24.04-x64` accept either `runtime_profile=full` or `runtime_profile=mini`, while the other currently enabled RIDs remain full-only. Then dispatch `pack.yml` with the same RID/profile, `validate_synthetic_runtime=false`, `publish_github_packages=false`, and `real_runtime_artifact_run_id=<runtime-input-run-id>`.
...
Ubuntu 22.04 ARM64 full 使用由 `ubuntu-24.04-arm` 宿主机编排的独立 `docker run` verifier。checkout 与同 run artifact 下载保留在原生 AArch64 宿主机，精确 artifact/provenance guard、隔离的 .NET 8 ARM64 restore/build、包内 loader 与确定性 DNN 则在 digest 固定的官方 Ubuntu 22.04 ARM64 userspace 内执行；不设置 `LD_LIBRARY_PATH`，也不使用 QEMU、`--platform` 或交叉编译。

Debian 12 ARM64 full 使用由原生 `ubuntu-24.04-arm` 宿主机编排的独立 `docker run` verifier。checkout 与精确同 run artifact 下载保留在 AArch64 宿主机；digest 固定的官方 Debian 12 ARM64 容器先核验 Debian 12、`aarch64`/`arm64`、glibc、PowerShell archive hash 与原生 .NET 8，再运行精确 artifact/provenance guard、隔离 restore/build、包内 loader 与确定性 DNN，且不设置 `LD_LIBRARY_PATH`。x64、QEMU、`--platform`、模拟、交叉构建和 ARM64 mini 均被排除。
...
要试跑真实 producer/consumer 链路，请先 dispatch `runtime-input.yml`；`win-x64`、`win-arm64` 与 `ubuntu.24.04-x64` 可选择 `runtime_profile=full` 或 `mini`，其他当前已启用 RID 仍只接受 `full`。随后 dispatch `pack.yml`，使用相同 RID/profile，并设置 `validate_synthetic_runtime=false`、`publish_github_packages=false` 和 `real_runtime_artifact_run_id=<runtime-input-run-id>`。
