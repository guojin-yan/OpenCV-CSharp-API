...
The mini profile builds and packages `core,imgproc,imgcodecs,videoio,geometry`. OpenCV 5 split geometry-backed contour, shape, and transform APIs out of imgproc, so `geometry` is required to keep the common imgproc wrapper useful; DNN, calib, features, photo, highgui, and other full-only modules remain excluded. The native wrapper compiles only common infrastructure plus core/imgproc/imgcodecs/videoio sources and exports a reduced compatibility ABI. APIs moved to excluded OpenCV 5 modules, including `GoodFeaturesToTrack` in `features`, keep their managed/native entrypoint shape but report `NOT_LINKED` under mini.

mini profile 会构建并打包 `core,imgproc,imgcodecs,videoio,geometry`。OpenCV 5 已把 imgproc 中基于 geometry 的轮廓、形状和变换 API 拆到 `geometry`，因此为了保留实用的常用 imgproc wrapper，mini 必须包含该依赖；DNN、calib、features、photo、highgui 等 full-only 模块仍被排除。native wrapper 只编译公共基础设施与 core/imgproc/imgcodecs/videoio 源，并导出缩减后的兼容 ABI。被 OpenCV 5 移到排除模块的 API（例如位于 `features` 的 `GoodFeaturesToTrack`）保持 managed/native 入口形状，但在 mini 下明确返回 `NOT_LINKED`。
