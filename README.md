...
`runtime-input.yml` is the first real producer workflow for that handoff. It currently produces `runtime-input-ubuntu.24.04-x64-full` on `ubuntu-24.04` and `runtime-input-ubuntu.22.04-x64-full` on `ubuntu-22.04`: each target fetches factual OpenCV source, builds OpenCV, links/tests `JYPPX.OpenCV.Native`, records distro/profile/build-list provenance, and uploads the agreed `runtime-input-<rid>-<profile>` layout. Mini and non-Ubuntu distro-specific Linux RID producers stay disabled until their linked-component or distro-native boundaries are implemented and verified.

`runtime-input.yml` 是该 handoff 的第一条真实 producer workflow。当前它会在 `ubuntu-24.04` 上生产 `runtime-input-ubuntu.24.04-x64-full`，并在 `ubuntu-22.04` 上生产 `runtime-input-ubuntu.22.04-x64-full`：每个目标都会获取事实性 OpenCV 源码、构建 OpenCV、链接并测试 `JYPPX.OpenCV.Native`，记录 distro/profile/build-list provenance，然后上传约定的 `runtime-input-<rid>-<profile>` layout。mini 和非 Ubuntu distro-specific Linux RID producer 会保持禁用，直到对应 linked-component 或 distro-native 边界完成并通过验证。
