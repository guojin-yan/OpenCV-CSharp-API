...
`runtime-input.yml` is the first real producer workflow for that handoff. It currently produces `runtime-input-ubuntu.24.04-x64-full` on `ubuntu-24.04`, `runtime-input-ubuntu.22.04-x64-full` on `ubuntu-22.04`, and `runtime-input-debian.12-x64-full` by running the build inside a `debian:12` container on an Ubuntu hosted runner. Each target fetches factual OpenCV source, builds OpenCV, links/tests `JYPPX.OpenCV.Native`, records distro/profile/build-list provenance, and uploads the agreed `runtime-input-<rid>-<profile>` layout. The Debian producer additionally records hosted-runner, container image, container distro/version, and libc evidence so it is not confused with a generic Ubuntu-hosted package-surface run. Mini and other distro-specific Linux RID producers stay disabled until their linked-component or distro-native boundaries are implemented and verified.

`runtime-input.yml` 是该 handoff 的第一条真实 producer workflow。当前它会在 `ubuntu-24.04` 上生产 `runtime-input-ubuntu.24.04-x64-full`，在 `ubuntu-22.04` 上生产 `runtime-input-ubuntu.22.04-x64-full`，并通过 Ubuntu hosted runner 中的 `debian:12` 容器生产 `runtime-input-debian.12-x64-full`。每个目标都会获取事实性 OpenCV 源码、构建 OpenCV、链接并测试 `JYPPX.OpenCV.Native`，记录 distro/profile/build-list provenance，然后上传约定的 `runtime-input-<rid>-<profile>` layout。Debian producer 还会记录 hosted runner、container image、container distro/version 和 libc 证据，避免把它混同为通用 Ubuntu-hosted package-surface run。mini 和其他 distro-specific Linux RID producer 会保持禁用，直到对应 linked-component 或 distro-native 边界完成并通过验证。
