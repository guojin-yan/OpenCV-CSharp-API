...
namespace JYPPX.OpenCvSharp.Samples.Case06KnnClassification
{
    internal static class Program
    {
        private static void Main(string[] args)
        {
            string outputDirectory = PublishedPackageSampleSupport.GetOutputDirectory(args, "published-package-ml");
            ...
            try
            {
                ...
                using (KNearest knn = KNearest.Create())
                {
                    ...
                    using (var grid = new Mat(gridHeight, gridWidth, MatType.CV_8UC3))
                    {
                        ...
                        panel = new Mat();
                        ImgProcCv2.Resize(grid, panel, new Size(PublishedPackageSampleSupport.PanelWidth,
                            PublishedPackageSampleSupport.PanelHeight), interpolation: InterpolationFlags.Nearest);
                    }
                    for (int i = 0; i < trainingLabels.Length; i++)
                    {
                        int x = (int)Math.Round(trainingValues[i * 2] * (PublishedPackageSampleSupport.PanelWidth - 1));
                        int y = (int)Math.Round(trainingValues[(i * 2) + 1] * (PublishedPackageSampleSupport.PanelHeight - 1));
                    ...
                    PublishedPackageSampleSupport.AddPanelLabel(panel, "06  KNN CLASSIFICATION", new Scalar(245, 245, 245));
                    PublishedPackageSampleSupport.AddMetric(panel, "3-nearest  3,600 queries");
                }
            }
            catch (OpenCvException exception) when (exception.Message.IndexOf("NOT_LINKED", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                mlLinked = false;
                queryCount = 0;
                panel = new Mat(PublishedPackageSampleSupport.PanelHeight, PublishedPackageSampleSupport.PanelWidth,
                ...
                PublishedPackageSampleSupport.AddMetric(panel, "Native ML  NOT_LINKED");
            }

            using (panel)
            {
                PublishedPackageSampleSupport.WritePng(outputDirectory, "knn-classification.png", panel);
            }
            PublishedPackageSampleSupport.WriteSummary("KNN classification", outputDirectory, mlLinked
